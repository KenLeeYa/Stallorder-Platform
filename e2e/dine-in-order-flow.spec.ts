...
test.beforeAll(async () => {
  ...
  if (staleShiftIds.length > 0) {
    await prisma.cashShiftReview.deleteMany({
      where: { cashShiftId: { in: staleShiftIds } },
    });
...
async function login(page: Page, email: string) {
  ...
  await expect(page).toHaveURL(
    /\/merchant\/dashboard\?organizationId=|\/staff\/|\/kitchen\?/,
  );
}
...
test("內用桌位從 QR 點餐連動廚房、出餐與折扣結帳", async ({
  browser,
  page,
}, testInfo) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Deep-Fried Chicken Cutlet" }),
  ).toBeVisible();
  ...
  await qrProductSelectionControl(
    sweetPotatoFries,
    "Sweet Potato Fries",
    "Increase Sweet Potato Fries",
  ).click();
  await sweetPotatoFries
    .getByRole("button", { name: "Add to cart", exact: true })
    .click();
  await expect(cartLines).toHaveCount(2);
  await expect(
    cartLines.filter({ hasText: "Deep-Fried Chicken Cutlet" }),
  ).toHaveCount(1);
  await expect(cartLines.filter({ hasText: "Sweet Potato Fries" })).toHaveCount(
    1,
  );
  await expectNoHorizontalOverflow(page);
  await captureMobileScreenshot(page, testInfo, "01-customer-qr-menu");
  await verifyCompactViewport(page, [
    page.getByTestId("qr-mobile-cart-summary"),
  ]);
  await page.getByTestId("qr-mobile-cart-summary").click();
  await expect(page.getByTestId("qr-cart-panel")).toHaveAttribute(
    "role",
    "dialog",
  );
  await page
  ...
  await page
    .getByRole("checkbox", { name: /I understand the estimated wait/ })
    .check();
  await expect(
  ...
  const confirmationResponse = staffPage.waitForResponse((response) => {
    ...
    try {
      return (
        (response.request().postDataJSON() as { status?: string }).status ===
        "CONFIRMED"
      );
    } catch {
      return false;
  ...
  await expect(
    checkout.getByRole("button", { name: "街口支付" }),
  ).toBeVisible();
  ...
  const rejectedCheckoutResponse = staffPage.waitForResponse(
    (response) =>
  ...
  await expect(
    checkout.getByRole("link", { name: "前往現金交班" }),
  ).toHaveAttribute("href", "/staff/aming-chicken/cash");
  const checkoutResponse = staffPage.waitForResponse(
    (response) =>
      response.url().includes("/api/stalls/aming-chicken/orders/") &&
      response.request().method() === "PATCH",
  );
  ...
  const finishCleaning = staffPage.getByRole("button", {
    name: "清潔完成，設為空桌",
  });
  ...
  await expect(
    merchantPage.getByRole("region", { name: "營運摘要" }),
  ).toBeVisible();
  ...
  await expect(
    merchantPage
      .getByRole("region", { name: "銷售摘要" })
      .getByTestId("sales-summary-dashboard"),
  ).toBeVisible();
  await expectNoHorizontalOverflow(merchantPage);
  await captureMobileScreenshot(
    merchantPage,
    testInfo,
    "07-merchant-sales-report",
  );
  await verifyCompactViewport(merchantPage, [
    merchantPage.getByRole("heading", { name: "銷售趨勢總覽", exact: true }),
    merchantPage
