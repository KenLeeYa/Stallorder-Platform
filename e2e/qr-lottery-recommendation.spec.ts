...
test("QR 抽抽樂先顯示動畫與結果視窗，取消不改購物車且不能再次抽取", async ({
  page,
}) => {
  ...
  await expect(resultDialog.getByTestId("lottery-result-basis")).toHaveText(
    "熱銷推薦",
  );
  await expect(resultDialog.getByTestId("lottery-discount-result")).toHaveText(
    "同時抽中 抽抽樂九折！",
  );
  await expect(page.getByTestId("qr-mobile-cart-summary")).toHaveCount(0);

  const acceptButton = resultDialog.getByRole("button", {
    name: "接受推薦",
    exact: true,
  });
  const cancelButton = resultDialog.getByRole("button", {
    name: "取消",
    exact: true,
  });
  ...
  await expect(page.getByTestId("qr-mobile-cart-summary")).toHaveCount(0);
  await expect(
    lottery.region.getByRole("button", { name: "今日已抽取", exact: true }),
  ).toBeFocused();
  expect(await page.evaluate(() => document.body.style.overflow)).not.toBe(
    "hidden",
  );

  const dailyLimitTrigger = lottery.region.getByRole("button", {
    name: "今日已抽取",
    exact: true,
  });
  ...
  await expect(dailyLimitDialog).toContainText(
    "今天的商品推薦與折扣結果已保留",
  );
  const acknowledgeDailyLimit = dailyLimitDialog.getByRole("button", {
    name: "我知道了",
    exact: true,
  });
...
test("reduced-motion 停止抽獎輪播與 CSS 動畫，回應完成後立即顯示結果", async ({
  page,
}) => {
...
for (const failureMode of ["HTTP_503", "NETWORK"] as const) {
  test(`抽獎 ${failureMode} 失敗後顯示可見錯誤、恢復焦點並可重試成功`, async ({
    page,
  }) => {
    const lottery = await mockLottery(
      page,
      SIMPLE_PRODUCT_ID,
      20,
      10 * 60_000,
    ...
    const drawButton = lottery.region.getByRole("button", {
      name: "開始抽抽樂",
      exact: true,
    });
    ...
    await expect(page.getByTestId("lottery-result-dialog")).toHaveCount(0);
    await expect(lottery.region.getByRole("alert")).toHaveText(
      "抽抽樂目前無法使用。",
    );
...
test("抽取結果顯示後即將逾時會先保留結果，關閉後由逾時視窗接管", async ({
  page,
}) => {
  await page.clock.install();
...
test("接受一般商品推薦後直接加入一份購物車", async ({ page }) => {
  ...
  await expect(page.getByTestId("qr-mobile-cart-summary")).toHaveCount(0);
  await resultDialog
    .getByRole("button", { name: "接受推薦", exact: true })
    .click();
...
test("接受需客製商品推薦後開啟規格視窗，但不先加入購物車", async ({ page }) => {
  ...
  await expect(
    configuration.getByRole("radio", { name: /加蛋/ }),
  ).toBeVisible();
  await expect(
    configuration.getByRole("button", { name: "加入購物車", exact: true }),
  ).toBeVisible();
  await expect(page.getByTestId("qr-mobile-cart-summary")).toHaveCount(0);
  expect(lottery.requestCount()).toBe(1);
});
...
async function drawAndWaitForResult(page: Page, lotteryRegion: Locator) {
  ...
  await expect(resultDialog).toHaveAttribute("data-phase", "drawing");
  await expect(
    resultDialog.getByTestId("lottery-product-carousel"),
  ).toBeVisible();
  await expect(resultDialog).toHaveAttribute("data-phase", "result", {
    timeout: 2_500,
  });
...
async function mockLottery(
  ...
  await page.route("**/create-order-session", (route) =>
    route.fulfill({
      status: 201,
      contentType: "application/json",
  ...
  await page.route("**/api/public/lottery-draw", async (route) => {
    ...
    const recommendedProduct =
      recommendedProductId === SIMPLE_PRODUCT_ID
        ? simpleProduct()
        : configurableProduct();
    return route.fulfill({
      status: 200,
      ...
      body: JSON.stringify({
        ...
        recommendationBasis: recommendedProduct.isBestSeller
          ? "BEST_SELLER"
          : "DISCOVERY",
