...
async function loginAsOwner(page: Page) {
  ...
  await expect(page).toHaveURL(/\/merchant\/dashboard\?organizationId=/u, {
    timeout: 30_000,
  });
}
...
test("已啟用的庫存、成長與菜單版本可由商家介面開啟", async ({ page }) => {
  ...
  await expect(page).toHaveURL(
    /\/merchant\/catalog\/versions\?organizationId=/u,
  );
  await expect(
    mainContent.getByRole("heading", { name: "菜單版本與發布", exact: true }),
  ).toBeVisible();
