...
test.describe("外帶 QR 人工核對與現金完成訂單", () => {
  ...
  test("真實 QR 下單經人工核對後，以現金完成訂單", async ({
    ...
    expect(createOrderResponse.request().postDataJSON()).toMatchObject({
      customerName: "",
      customerPhone: "",
    });
    ...
    try {
      ...
      expect(preparingResponse.request().postDataJSON()).toEqual({
        status: "PREPARING",
      });
      ...
      await pickupCheckout
        .getByRole("button", { name: "無法取得取餐碼", exact: true })
        .click();
      const manualPickupDialog = staffPage.getByRole("alertdialog", {
        name: "人工核對取餐",
      });
      await expect(manualPickupDialog).toContainText(`訂單 ${orderNo}`);
      await expect(
        manualPickupDialog.getByLabel("輸入完整訂單編號以確認"),
      ).toHaveCount(0);
      ...
      await manualPickupDialog
        .getByLabel("已向顧客核對稱呼與全部餐點內容")
        .check();
      ...
      createdOrderId =
        (
          await prisma.order.findUnique({
            where: { stallId_orderNo: { stallId, orderNo: createdOrderNo } },
            select: { id: true },
          })
        )?.id ?? "";
