...
test("分享外帶連結依取餐時段更新商品與套餐選項並清除失效購物車", async ({
  page,
}) => {
  ...
  await page.route("**/create-order-session", async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        expiresAt: new Date(
          Date.now() + (sessionExpiresSoon ? 30_000 : 10 * 60_000),
        ).toISOString(),
  ...
  await expect(
    page.getByRole("heading", { name: "預約時段測試攤位" }),
  ).toBeVisible();
  await expect(
    page.getByText("目前為非營業時間，僅接受外帶自取預約。", { exact: true }),
  ).toBeVisible();
  const initialPickupSlot = fulfillmentSlot(firstPickupSlot);
  const preorderFields = page.getByTestId(
    "qr-preorder-fulfillment-time-fields",
  );
  ...
  await expect(
    page.getByText("請先確認並套用預約取餐時間，完成後才會顯示可點商品。", {
      exact: true,
    }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("article").filter({ hasText: "早鳥飯糰" }),
  ).toBeVisible();
  await expect(
    page.getByRole("article").filter({ hasText: "晚場便當" }),
  ).toHaveCount(0);
  await expect(page.getByRole("region", { name: "抽抽樂推薦" })).toHaveCount(0);

  const bundle = page.locator("article#qr-product-slot-bundle");
  await qrProductSelectionControl(bundle, "時段限定套餐").click();
  ...
  await qrProductSelectionControl(morningItem, "早鳥飯糰").click();
  ...
  await expect(
    page.getByText("尚未套用新的取餐時間；套用後才會更新可點商品與購物車。", {
      exact: true,
    }),
  ).toBeVisible();
  ...
  await expect(checkoutBlocker).toHaveText(
  ...
  await expect(
    bundle.getByRole("radio", { name: /晚場綠茶/ }),
  ).not.toBeChecked();
  ...
  await expect(bundle.getByRole("radio", { name: /晨間紅茶/ })).toBeVisible();
  await expect(
    bundle.getByRole("radio", { name: /晨間紅茶/ }),
  ).not.toBeChecked();
  await expect(cart).toContainText("共 1 份");
  await expect(cart).toContainText("$150");
  await bundle
  ...
  await bundle
    ...
    .getByRole("button", { name: "關閉" })
    .click();
  const bundleCartLine = page
    .getByTestId("qr-cart-line")
    .filter({ hasText: "時段限定套餐" });
  await bundleCartLine
    .getByRole("button", { name: "移除", exact: true })
    .click();
  const laterItem = page.locator("article#qr-product-later-item");
  await qrProductSelectionControl(laterItem, "晚場便當").click();
    ...
    .poll(() =>
      page.evaluate(() => {
        const raw = Object.entries(window.localStorage).find(
          ([key]) =>
            key.startsWith("stallorder_qr_cart:") && key.endsWith(":preorder"),
        )?.[1];
        if (!raw) return null;
  ...
  await expect(preorderDate).toHaveValue(
    fulfillmentSlot(secondPickupSlot).date,
  );
  await expect(preorderHour).toHaveValue(
    fulfillmentSlot(secondPickupSlot).hour,
  );
  await expect(preorderMinute).toHaveValue(
    fulfillmentSlot(secondPickupSlot).minute,
  );
...
test("營業中的實體 QR 不顯示取餐時間且送單固定為即時外帶", async ({ page }) => {
  ...
  await qrProductSelectionControl(product, "外帶測試餐").click();
  await page.getByTestId("qr-mobile-cart-summary").click();
  await expect(
    page.getByRole("radio", { name: "儘快，不指定時間" }),
  ).toHaveCount(0);
  await expect(page.getByRole("radio", { name: "指定取餐時間" })).toHaveCount(
    0,
  );
  await expect(
    page.locator('[data-testid$="fulfillment-time-fields"]'),
  ).toHaveCount(0);
  expect(
    await page.evaluate(
      () =>
