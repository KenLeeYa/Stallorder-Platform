...
async function expectMobileSheetActionAvailable(
  ...
  await expect
    .poll(() =>
      sheet.evaluate((element) => element.scrollWidth <= element.clientWidth),
    )
    .toBe(true);
  await expect
    .poll(() =>
...
test("QR 同商品可加入兩個不同註記列，報價低頻更新且返回頂端不遮住手機購物車", async ({
  page,
}) => {
  ...
  await page.route("**/create-order-session", async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        expiresAt:
          requestBody.includeMenu !== false && extendSessionOnReload
  ...
  await qrProductSelectionControl(product, "測試雞排").click();
  const firstConfiguration = product.getByRole("dialog", { name: "測試雞排" });
  const addFirstLine = firstConfiguration.getByRole("button", {
    name: "加入購物車",
    exact: true,
  });
  await expect(addFirstLine).toBeDisabled();
  await expect(firstConfiguration.getByRole("status")).toHaveText(
    "請完成「測試雞排」的必選註記。",
  );
  for (const width of [360, 390]) {
    await page.setViewportSize({ width, height: 500 });
    await expectMobileSheetActionAvailable(
      page,
      firstConfiguration,
      addFirstLine,
    );
  ...
  await expect(qrProductSelectionControl(product, "測試雞排")).toBeVisible();
  await qrProductSelectionControl(product, "測試雞排").click();
  ...
  const continueToCheckout = cartPanel.getByRole("button", {
    name: "繼續填寫訂購資料",
    exact: true,
  });
  for (const width of [360, 390]) {
  ...
  const finishEditing = product.getByRole("button", {
    name: "修改完成",
    exact: true,
  });
  ...
  await expect
    .poll(() =>
      finishEditing.evaluate((element) => {
        const rect = element.getBoundingClientRect();
        const target = document.elementFromPoint(
          rect.left + rect.width / 2,
          rect.top + rect.height / 2,
  ...
  await mobileSummary.click();
  await expect(cartLines).toHaveCount(2);
  await expect(cartLines.nth(0)).toHaveAttribute(
    "data-cart-line-id",
    firstLineId!,
  );
  ...
  await product.getByRole("radio", { name: /加蛋/ }).check();
  await expect(
    product.getByRole("button", { name: "加入購物車", exact: true }),
  ).toBeVisible();
  await product
    .getByRole("dialog", { name: "測試雞排" })
    .getByRole("button", { name: "關閉" })
    .click();

  const backToTop = page.getByTestId("qr-back-to-top");
  await expect(backToTop).toBeVisible();
  const [backBox, summaryBox] = await Promise.all([
    backToTop.boundingBox(),
    mobileSummary.boundingBox(),
  ]);
  expect(
    backBox && summaryBox && backBox.y + backBox.height <= summaryBox.y,
  ).toBe(true);
  await mobileSummary.click();
  ...
  const [desktopBackBox, desktopCartBox] = await Promise.all([
    backToTop.boundingBox(),
    desktopCart.boundingBox(),
  ]);
  expect(
    desktopBackBox &&
      desktopCartBox &&
      desktopBackBox.x + desktopBackBox.width <= desktopCartBox.x,
  ).toBe(true);
  await page.setViewportSize({ width: 390, height: 500 });
  ...
  await expect(
    page.getByText("目前預估等候時間：12～18 分鐘", { exact: true }),
  ).toBeVisible();
  await mobileSummary.click();
  await page
    ...
    .click();
  await expect(
    page.getByRole("heading", { name: "訂購資料與確認", exact: true }),
  ).toBeFocused();
  ...
  await expect(
    expiryDialog.getByRole("button", { name: "重新整理並繼續點餐" }),
  ).toBeFocused();
  ...
  expect(
    await page.evaluate((token) => {
      const stored = window.localStorage.getItem(
        `stallorder_qr_cart:${encodeURIComponent(token)}`,
      );
      return stored ? JSON.parse(stored).lines.length : 0;
    }, "independent-lines-e2e-token"),
  ).toBe(2);
  ...
  await Promise.all([
    ...
    page
      .getByRole("dialog", { name: "已逾時" })
      .getByRole("button", { name: "重新整理並繼續點餐" })
      .click(),
  ]);
  await expect(
    page.getByText("已恢復上次尚未送出的點餐內容。", { exact: true }),
  ).toBeVisible();
