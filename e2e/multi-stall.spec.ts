...
test.describe("多攤位商戶關鍵流程", () => {
  ...
  test.beforeAll(async () => {
    ...
    await prisma.authSession.deleteMany({
      where: {
        profile: {
          email: { in: [ownerEmail, staffEmail, kitchenEmail, financeEmail] },
        },
      },
    });
    ...
    isolatedBillingPeriodEnd.setUTCDate(
      isolatedBillingPeriodEnd.getUTCDate() + 30,
    );
  ...
  test("Google 登入 owner 並建立第二攤位", async ({ page }) => {
    ...
    if (process.env.PLAYWRIGHT_PRODUCTION_SERVER !== "true") {
      const warmupResponse = await page
        .context()
        .request.get(createStallApiPath);
    ...
    await gotoLocalPath(
      page,
      `/merchant/stalls/new?organizationId=${organization.id}`,
    );
    ...
    const createStallResponse = page.waitForResponse(
      (response) =>
    ...
    await expect(page).toHaveURL(
      /\/merchant\/stalls\/[0-9a-f-]+\?organizationId=/,
    );
  ...
  test("共用商品分派、攤位覆寫價格與顧客菜單一致", async ({ page }) => {
    ...
    await gotoLocalPath(
      page,
      `/merchant/catalog?organizationId=${organization.id}`,
    );
    const productActions = await openSharedCatalogProductActions(
      page,
      sharedProductName,
    );
    const openAssignmentsButton = productActions.getByRole("button", {
      name: "分派攤位",
    });
    await expect(openAssignmentsButton).toBeEnabled();
    ...
    await expect(saveAssignmentsButton).toBeEnabled();
    ...
    await expect(
      page.getByText("攤位分派已更新。", { exact: true }),
    ).toBeVisible();
    await expect
      .poll(() =>
    ...
    const saveProduct = productRow.getByRole("button", {
      name: `儲存 ${sharedProductName}`,
    });
    await expect(saveProduct).toBeEnabled();
    const saveProductResponse = page.waitForResponse(
      (response) =>
        new URL(response.url()).pathname.startsWith(
          `/api/merchant/stalls/${secondStall.id}/products/`,
        ) &&
  ...
  test("儀表板範圍、staff、finance、kitchen 與跨組織 URL 權限", async ({
    ...
    await expect(
      stallDialog.getByRole("button", { name: "全部攤位", exact: true }),
    ).toHaveAttribute("aria-current", "page");
    await expect(
      stallDialog.getByRole("button", { name: firstStall.name, exact: true }),
    ).toBeVisible();
    await expect(
      stallDialog.getByRole("button", { name: secondStall.name, exact: true }),
    ).toBeVisible();
    await stallDialog
      .getByRole("button", { name: firstStall.name, exact: true })
      .click();
    await expect(page).toHaveURL(new RegExp(`/merchant/${firstStall.slug}$`));
    await selectCompactOption(
      page,
      `選擇攤位：${firstStall.name}`,
      "選擇攤位",
      "全部攤位",
    );
    await expect(page).toHaveURL(
    ...
    await expect(
      page.getByRole("link", { name: secondStall.name, exact: true }),
    ).toBeVisible();

    await openCompactSwitcher(page, "商家管理", "切換工作模式");
    const workModeDialog = page.getByRole("dialog", { name: "切換工作模式" });
    await expect(
      workModeDialog.getByRole("button", { name: "商家管理", exact: true }),
    ).toHaveAttribute("aria-current", "page");
    await expect(
      workModeDialog.getByRole("button", {
        name: `店員 · ${firstStall.name}`,
        exact: true,
      }),
    ).toBeVisible();
    await expect(
      workModeDialog.getByRole("button", {
        name: `廚房 · ${firstStall.name}`,
        exact: true,
      }),
    ).toBeVisible();
    await workModeDialog
      .getByRole("button", { name: `店員 · ${firstStall.name}`, exact: true })
      .click();
    await expect(page).toHaveURL(new RegExp(`/staff/${firstStall.slug}$`));
    await expect(
      page.getByRole("heading", { name: firstStall.name, exact: true }),
    ).toBeVisible();
    await expect(
    ...
    await expect(page).toHaveURL(
      new RegExp(`/kitchen\\?stall=${firstStall.slug}$`),
    );
    await expect(
      page.getByRole("heading", { name: "廚房生產系統" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", {
        name: `廚房 · ${firstStall.name}`,
    ...
    await gotoLocalPath(
      page,
      `/merchant/dashboard?organizationId=${organization.id}&stallId=${secondStall.id}`,
    );
    ...
    await expect(
      comparisonTable.getByRole("link", {
        name: secondStall.name,
        exact: true,
      }),
    ).toBeVisible();
    await expect(
      comparisonTable.getByRole("link", { name: firstStall.name, exact: true }),
    ).toHaveCount(0);

    const crossOrganizationResponse = await page.goto(
      `/merchant/stalls/${otherStall.id}`,
    );
    ...
    if (process.env.PLAYWRIGHT_PRODUCTION_SERVER !== "true") {
      const warmupResponse = await page
        .context()
        .request.get(crossOrganizationOverviewPath);
    ...
    await expect(page.getByRole("button", { name: /^選擇攤位/u })).toHaveCount(
      0,
    );
    const unassignedStaffResponse = await page.goto(
      `/staff/${secondStall.slug}`,
    );
    ...
    await expect(
      page.getByRole("heading", { name: "找不到此頁面", exact: true }),
    ).toBeVisible();
    await expect(page.getByText(secondStall.name, { exact: true })).toHaveCount(
      0,
    );
    ...
    if (process.env.PLAYWRIGHT_PRODUCTION_SERVER !== "true") {
      const warmupResponse = await page
        .context()
        .request.get(financeOrderApiPath);
      expect(warmupResponse.status()).toBe(405);
      await warmupResponse.dispose();
    }
    const financeMutation = await page.evaluate(
      async ({ stallSlug, orderId }) => {
        ...
        const response = await fetch(
          `/api/stalls/${stallSlug}/orders/${orderId}`,
          {
            method: "PATCH",
            headers: {
    ...
    await expect(
      page.getByRole("heading", { name: "付款分析", exact: true }),
    ).toHaveCount(0);
  });
  ...
  test("多攤位介面在手機寬度無水平溢出", async ({ page }) => {
    ...
    await expect(
      page.getByRole("navigation", { name: "商戶功能" }),
    ).toBeVisible();
    ...
    const [brandBox, appStatusBox] = await Promise.all([
      brand.boundingBox(),
      appStatus.boundingBox(),
    ]);
    ...
    await expect(
      page.getByRole("button", { name: /^(?:展開|收合)商戶選項$/u }),
    ).toHaveCount(0);
    await expect(
      page.getByRole("navigation", { name: "商戶功能" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "選擇攤位：全部攤位", exact: true }),
    ).toBeVisible();
    await expect(page.getByRole("heading", { name: "攤位比較" })).toBeVisible();
    await expect(
      page.getByRole("article").filter({ hasText: secondStall.name }),
    ).toBeVisible();
    const summaryDashboard = page
      .locator("#main-content")
      .getByLabel("營運摘要");
    const summaryColumnCount = await summaryDashboard.evaluate(
      (element) =>
        window.getComputedStyle(element).gridTemplateColumns.split(" ").length,
    );
    expect(summaryColumnCount).toBe(2);
    await expect(summaryDashboard.locator(":scope > div").nth(6)).toHaveCSS(
      "grid-column-end",
      "auto",
    );
    ...
    const narrowSummaryColumnCount = await summaryDashboard.evaluate(
      (element) =>
        window.getComputedStyle(element).gridTemplateColumns.split(" ").length,
    );
    ...
    expect(narrowDimensions.document).toBeLessThanOrEqual(
      narrowDimensions.viewport + 1,
    );
    expect(narrowDimensions.body).toBeLessThanOrEqual(
      narrowDimensions.viewport + 1,
    );

    await page.setViewportSize({ width: 768, height: 1024 });
    await expect(summaryDashboard).toBeVisible();
    const tabletSummaryColumnCount = await summaryDashboard.evaluate(
      (element) =>
        window.getComputedStyle(element).gridTemplateColumns.split(" ").length,
    );
    ...
    expect(tabletDimensions.document).toBeLessThanOrEqual(
      tabletDimensions.viewport + 1,
    );
    expect(tabletDimensions.body).toBeLessThanOrEqual(
      tabletDimensions.viewport + 1,
    );
  });

  test("手機 Header 以 server route scope 覆蓋 stale 組織 query 並拒絕跨 tenant 攤位", async ({
    page,
  }) => {
    test.setTimeout(180_000);
    await page.setViewportSize({ width: 390, height: 844 });
    ...
    await page.evaluate(
      ({ organizationId }) => {
        window.localStorage.setItem(
          "stallorder.organization.preference",
          organizationId,
        );
      },
    ...
    await expect(page.getByText(otherStall.name, { exact: true })).toHaveCount(
      0,
    );
    await expect(page.getByText("E2E 隔離組織", { exact: true })).toHaveCount(
      0,
    );
...
async function deleteTestOrganizations(args: {
...
async function expectRenderedMerchantScope(
  ...
  await expect(
    organizationDialog.getByRole("button", {
      name: expected.businessName,
      exact: true,
    }),
  ).toHaveAttribute("aria-current", "page");
  await expect(
    organizationDialog.getByText(expected.unauthorizedBusinessName, {
      exact: true,
    }),
  ).toHaveCount(0);
  ...
  await expect(
    page.getByRole("link", { name: "攤點通", exact: true }),
  ).toHaveAttribute(
    "href",
  ...
  if (await stallTrigger.count()) {
    ...
    const stallDialog = page.getByRole("dialog", { name: "選擇攤位" });
    await expect(
      stallDialog.getByRole("button", {
        name: expected.stall.name,
        exact: true,
      }),
    ).toHaveAttribute("aria-current", "page");
    await page.keyboard.press("Escape");
      ...
      () =>
        control.evaluate(
          (element, options) => {
            ...
            const propsKey = Object.keys(eventTarget).find((key) =>
              key.startsWith("__reactProps$"),
            );
            if (!propsKey) return false;
            const props = (eventTarget as unknown as Record<string, unknown>)[
              propsKey
            ];
            return (
              typeof props === "object" &&
              props !== null &&
              typeof (props as Record<string, unknown>)[options.handler] ===
                "function"
            );
          },
          { handler, target },
...
async function openCustomerMenu(
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      await expect(
        page.getByRole("heading", { name: stallName, exact: true }),
