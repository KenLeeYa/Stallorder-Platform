...
test.afterAll(async () => {
  ...
  if (createdCashShiftId) {
    await prisma.cashShiftReview.deleteMany({
      where: { cashShiftId: createdCashShiftId },
    });
...
test("內用顧客名稱與桌位欄位在桌面版對齊", async ({ page }, testInfo) => {
  ...
  expect(
    await functionGrid.evaluate((element) => {
  ...
  await expect(
    page.getByRole("dialog", { name: "離線裝置", exact: true }),
  ).toBeVisible();
  ...
  expect(
    await groupNavigation.evaluate(
      (element) => window.getComputedStyle(element).position,
    ),
  ).toBe("sticky");
  expect(
    await menuPanel.evaluate(
  ...
  expect(
    Math.abs(
      cartPositionAfterCategoryJump!.y - cartPositionBeforeCategoryJump!.y,
    ),
  ).toBeLessThanOrEqual(1);
  await friedGroup
    .getByRole("button", { name: "收合商品群組 人氣炸物", exact: true })
    .click();
  await expect(
    friedGroup.getByRole("button", {
      name: "展開商品群組 人氣炸物",
      exact: true,
    }),
  ...
  await expect(
    drinkGroup.getByTestId("staff-product-card").first(),
  ).toBeVisible();
  ...
  await expect(
    friedGroup.getByTestId("staff-product-card").first(),
  ).toBeVisible();
  ...
  expect(Math.abs(customerBox!.height - tableBox!.height)).toBeLessThanOrEqual(
    1,
  );
  await page.screenshot({
    path: testInfo.outputPath("staff-pos-dine-in-alignment.png"),
    fullPage: true,
  });
});
...
test("店員內用與外送使用獨立設定，且建立訂單時重新驗證", async ({ page }) => {
  ...
  try {
    ...
    await page.goto("/staff/aming-chicken");
    await dismissStaffStartReminder(page);
    const initialConfigurationPromise = page.waitForResponse(
      (response) =>
        new URL(response.url()).pathname.endsWith(
          "/api/stalls/aming-chicken/pos-configuration",
        ) && response.request().method() === "GET",
    ...
    const dialog = page.getByRole("dialog", { name: "店員點餐" });
    const dineInButton = dialog.getByRole("button", {
      name: "內用",
      exact: true,
    });
    const deliveryButton = dialog.getByRole("button", {
      name: "外送",
      exact: true,
    });
    ...
    await expect(
      dialog.getByLabel("聯絡電話（選填）", { exact: true }),
    ).toHaveValue("");
    await expect(
    ...
    const rejectedDineInPromise = page.waitForResponse(
      (response) =>
        response.url().endsWith("/api/stalls/aming-chicken/orders") &&
        response.request().method() === "POST",
    );
    ...
    expect(await rejectedDineIn.json()).toMatchObject({
    ...
    const refreshedConfigurationPromise = page.waitForResponse(
      (response) =>
        new URL(response.url()).pathname.endsWith(
          "/api/stalls/aming-chicken/pos-configuration",
        ) && response.request().method() === "GET",
    ...
    await expect(
      refreshedDialog.getByRole("button", { name: "內用", exact: true }),
    ).toBeEnabled();
    await expect(
      refreshedDialog.getByRole("button", { name: "外送", exact: true }),
    ).toBeDisabled();
  } finally {
    await prisma.stallOrderingSettings.update({
      where: { stallId },
...
test("店員可在手機介面代客點餐並立即完成收款", async ({ page }, testInfo) => {
  ...
  expect(
    await functionGrid
      .locator(":scope > [data-testid^='staff-function-']")
      .count(),
  ).toBe(4);
  expect(
  ...
  expect(await functionGrid.locator(":scope > *").count()).toBeLessThanOrEqual(
    12,
  );
  const [staffOrderBox, floorPlanBox] = await Promise.all([
    functionGrid
      .getByRole("button", { name: "店員點餐", exact: true })
      .boundingBox(),
    functionGrid
      .getByRole("link", { name: "桌位平面圖", exact: true })
      .boundingBox(),
  ]);
  expect(staffOrderBox).not.toBeNull();
  expect(floorPlanBox).not.toBeNull();
  expect(
    Math.abs(staffOrderBox!.width - floorPlanBox!.width),
  ).toBeLessThanOrEqual(1);
  expect(
  ...
  if (!targetPickupSlot)
    throw new Error("店員指定時間 E2E 找不到 5 分鐘單位的可用時段。");
  ...
  await expect
    ...
    .toBeGreaterThan(0);
  await expect(
    dialog.getByRole("radio", { name: "儘快，不指定時間" }),
  ).toBeChecked();
  await expect(
    dialog.getByTestId("staff-takeout-fulfillment-time-fields"),
  ).toHaveCount(0);
  await dialog.getByRole("radio", { name: "指定取餐時間" }).check();

  const pickupFields = dialog.getByTestId(
    "staff-takeout-fulfillment-time-fields",
  );
  ...
  await expect(pickupDate).toHaveAttribute(
    "min",
    fulfillmentTimeSlots[0]!.date,
  );
  await expect(pickupDate).toHaveAttribute(
    "max",
    fulfillmentTimeSlots.at(-1)!.date,
  );
  ...
  expect(
    (await pickupHour.locator("option").allTextContents()).every((hour) =>
  ...
  expect(
    (await pickupMinute.locator("option").allTextContents()).every(
      (minute) => /^(?:[0-5]\d)$/.test(minute) && Number(minute) % 5 === 0,
    ),
  ).toBe(true);
  ...
  const deliveryFields = dialog.getByTestId(
    "staff-delivery-fulfillment-time-fields",
  );
  ...
  await expect(
    dialog.getByTestId("staff-delivery-fulfillment-time-fields"),
  ).toHaveCount(0);
  await expect(
    dialog.getByRole("radio", { name: "儘快，不指定時間" }),
  ).toBeChecked();
  await dialog.getByRole("radio", { name: "指定取餐時間" }).check();
  const resetPickupFields = dialog.getByTestId(
    "staff-takeout-fulfillment-time-fields",
  );
  await resetPickupFields.getByLabel("取餐日期").fill(targetPickupSlot.date);
  await resetPickupFields
    .getByLabel("取餐時間－時")
    .selectOption(targetPickupSlot.hour);
  await resetPickupFields
    .getByLabel("取餐時間－分")
  ...
  const storedDraft = await page.evaluate(() => {
    const entry = Object.entries(window.localStorage).find(([key]) =>
      key.startsWith("stallorder_staff_order_drafts:"),
    );
    if (!entry) return null;
    const drafts = JSON.parse(entry[1]) as Array<{
      label: string;
      cartDraft: string;
  ...
  await draftManager
    .getByRole("button", { name: "繼續點餐", exact: true })
    .click();
  ...
  await page
    .getByRole("dialog", { name: "此裝置的暫存訂單" })
    .getByRole("button", { name: `刪除暫存單 ${storedDraft?.first?.label}` })
    .click();
  await expect(
    page
      .getByRole("dialog", { name: "此裝置的暫存訂單" })
      .getByTestId("staff-draft-card"),
  ).toHaveCount(0);
  ...
  await expect(
    cartPanel.getByTestId("staff-order-checkout-controls"),
  ).toBeVisible();
  for (const touchTarget of [
    ...
    await expect
      .poll(async () => (await touchTarget.boundingBox())?.height ?? 0)
      .toBeGreaterThanOrEqual(44);
  }
  ...
  await page.screenshot({
    path: testInfo.outputPath("staff-pos-mobile.png"),
    fullPage: true,
  });
  ...
  await page.reload();
  await expect(staffMain).toHaveCount(1);
  const futureOrdersToggle = staffMain.locator(
    'button[aria-controls="future-scheduled-orders"]:visible',
  );
  ...
  await expect(
    page
      .locator("article")
      .filter({ hasText: `訂單 ${payload.order.orderNo}` }),
  ).toHaveCount(0);
  ...
  const futureTicket = futureOrders
    .locator("article")
    .filter({ hasText: `訂單 ${payload.order.orderNo}` });
  ...
  await expect(
    futureTicket.getByRole("button", {
      name: /開始製作|全部開始製作|完成訂單/,
    }),
  ).toHaveCount(0);
  ...
  const prematureProduction = await page.evaluate(async (orderId) => {
    ...
    const response = await fetch(
      `/api/stalls/aming-chicken/orders/${orderId}`,
      {
        method: "PATCH",
...
test("店員可將同商品不同註記分列加入購物車，並移除選錯的獨立列", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1024, height: 900 });
  await login(page);
  ...
  await configurator.getByRole("checkbox", { name: /加蛋/ }).click();
  await configurator
    .getByRole("button", { name: "加入購物車", exact: true })
    .click();
  await expect(
    product.getByText("購物車已有 1 份", { exact: true }),
  ).toBeVisible();
  ...
  await openConfigurator.click();
  await configurator.getByRole("checkbox", { name: /加起司/ }).click();
  await configurator
    .getByRole("button", { name: "加入購物車", exact: true })
    .click();
  ...
  const removeCheese = cheeseLine.getByRole("button", {
    name: "移除 香酥雞排（加起司）",
  });
  await expect
    .poll(async () => (await removeCheese.boundingBox())?.height ?? 0)
    .toBeGreaterThanOrEqual(44);
  ...
  const editableLineId =
    await editableCheeseLine.getAttribute("data-cart-line-id");
  expect(editableLineId).not.toBeNull();
  const friedGroupToggle = dialog.getByRole("button", {
    name: "收合商品群組 人氣炸物",
    exact: true,
  });
  await friedGroupToggle.click();
  await expect(
    dialog.getByRole("button", { name: "展開商品群組 人氣炸物", exact: true }),
  ).toHaveAttribute("aria-expanded", "false");
  await expect(product).toHaveCount(0);
  await editableCheeseLine
    .getByRole("button", { name: "修改客製", exact: true })
    .click();
  await expect(
    dialog.getByRole("button", { name: "展開商品群組 人氣炸物", exact: true }),
  ).toHaveAttribute("aria-expanded", "false");
  ...
  await expect(
    configurator.getByRole("checkbox", { name: /加起司/ }),
  ).toHaveAttribute("aria-checked", "true");
  await configurator.getByTitle("增加 香酥雞排").click();
  await expect(editableCheeseLine).toContainText("1 × 香酥雞排");
  await configurator.getByRole("checkbox", { name: /加蛋/ }).click();
  await expect(
    configurator.getByRole("button", { name: "修改完成", exact: true }),
  ).toBeVisible();
  await configurator
    .getByRole("button", { name: "修改完成", exact: true })
    .click();
  await expect(cartLines).toHaveCount(2);
  const editedLine = dialog.locator(
    `[data-testid="staff-cart-line"][data-cart-line-id="${editableLineId!}"]`,
  );
  ...
  await editedLine
    .getByRole("button", { name: "修改客製", exact: true })
    .click();
  await configurator.getByTitle("減少 香酥雞排").click();
  await expect(editedLine).toContainText("2 × 香酥雞排");
  await configurator
    .getByRole("button", { name: "修改完成", exact: true })
    .click();
  ...
  expect(requestItems.map((item) => item.noteOptionIds.length).sort()).toEqual([
    1, 2,
  ]);
  expect(requestItems[0]?.noteOptionIds).not.toEqual(
    requestItems[1]?.noteOptionIds,
  );
  ...
  expect(
    stored.items
      .flatMap((item) => item.noteOptions.map((option) => option.optionName))
      .sort(),
  ).toEqual(["加蛋", "加蛋", "加起司"]);
});

test("LINE 固定外送網址可指定送達時間，店家提議後由顧客確認", async ({
  browser,
  page,
}, testInfo) => {
  ...
  try {
    ...
    await expect(
      settingsPage.getByRole("heading", { name: "線上外送", exact: true }),
    ).toBeVisible();
    await expect(
      settingsPage.getByRole("switch", { name: /線上外送/u }),
    ).toHaveAttribute("aria-checked", "true");
  } finally {
  ...
  await expect(
    page.getByRole("main").getByText("外送", { exact: true }),
  ).toBeVisible();
  await expect(page.getByTestId("qr-cart-panel")).toBeHidden();
  await expect(page.locator("[data-nextjs-dialog]")).toHaveCount(0);
  await page.screenshot({
    path: testInfo.outputPath("line-delivery-mobile.png"),
    fullPage: true,
  });
  ...
  await qrProductSelectionControl(deliveryProduct, "香酥雞排").click();
  const requiredGroups = page
    .locator("fieldset")
    .filter({ has: page.locator("legend") });
  ...
  await expect(
    page.getByRole("radio", { name: "儘快，不指定時間" }),
  ).toBeChecked();
  await page.getByRole("radio", { name: "指定送達時間" }).check();
  const deliveryFields = page.getByTestId(
    "qr-delivery-fulfillment-time-fields",
  );
  const deliveryDate = deliveryFields.getByLabel("送達日期");
  const deliveryHour = deliveryFields.getByLabel("送達時間－時");
  const deliveryMinute = deliveryFields.getByLabel("送達時間－分");
  ...
  expect(
    (await deliveryHour.locator("option").allTextContents()).every((hour) =>
      /^(?:[01]\d|2[0-3])$/.test(hour),
    ),
  ).toBe(true);
  expect(
    (await deliveryMinute.locator("option").allTextContents()).every(
      (minute) => /^(?:[0-5]\d)$/.test(minute) && Number(minute) % 5 === 0,
    ),
  ).toBe(true);
  expect(
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        const raw = Object.entries(window.localStorage).find(
          ([key]) =>
            key.startsWith("stallorder_qr_cart:") && key.endsWith(":delivery"),
        )?.[1];
        if (!raw) return null;
  ...
  await page.reload();
  await page.getByTestId("qr-mobile-cart-summary").click();
  const restoredDeliveryCartPanel = page.getByRole("dialog", {
    name: "您的訂單",
  });
  ...
  const restoredDeliveryFields = restoredDeliveryCartPanel.getByTestId(
    "qr-delivery-fulfillment-time-fields",
  );
  await expect(restoredDeliveryFields.getByLabel("送達日期")).toHaveValue(
    requestedSlot.date,
  );
  await expect(restoredDeliveryFields.getByLabel("送達時間－時")).toHaveValue(
    requestedSlot.hour,
  );
  await expect(restoredDeliveryFields.getByLabel("送達時間－分")).toHaveValue(
    requestedSlot.minute,
  );
  await expect(restoredDeliveryCartPanel.getByLabel("顧客稱呼")).toHaveValue(
    customerName,
  );
  await expect(restoredDeliveryCartPanel.getByLabel("聯絡電話")).toHaveValue(
    "0912345678",
  );
  await expect(restoredDeliveryCartPanel.getByLabel("外送地址")).toHaveValue(
    deliveryAddress,
  );
  const submit = restoredDeliveryCartPanel.getByRole("button", {
    name: "送出訂單",
    exact: true,
  });
  ...
  try {
    ...
    const proposalDialog = staffPage.getByRole("dialog", {
      name: "提出新的送達時間",
    });
    const proposalFields = proposalDialog.getByTestId(
      "staff-time-proposal-fields",
    );
    ...
    await expect(proposalDate).toHaveAttribute(
      "min",
      fulfillmentSlots[0]!.date,
    );
    await expect(proposalDate).toHaveAttribute(
      "max",
      fulfillmentSlots.at(-1)!.date,
    ...
    expect(
      (await proposalHour.locator("option").allTextContents()).every((hour) =>
        /^(?:[01]\d|2[0-3])$/.test(hour),
      ),
    ).toBe(true);
    expect(
      (await proposalMinute.locator("option").allTextContents()).every(
        (minute) => /^(?:[0-5]\d)$/.test(minute) && Number(minute) % 5 === 0,
      ),
    ).toBe(true);
    await proposalDialog
      .getByLabel("調整原因")
      .fill("目前訂單較多，建議延後送達");
    const proposalResponsePromise = staffPage.waitForResponse(
      (response) =>
        new URL(response.url()).pathname.endsWith(
          `/orders/${order.id}/fulfillment-time`,
        ) && response.request().method() === "PATCH",
    );
    await proposalDialog
      .getByRole("button", { name: "通知顧客確認", exact: true })
      .click();
    ...
    const customerTimePanel = page.getByRole("region", {
      name: "預計送達時間",
    });
    await expect(
      customerTimePanel.getByText("等待您確認新時間", { exact: true }),
    ).toBeVisible();
    ...
    await customerTimePanel
      .getByRole("button", { name: "接受新送達時間", exact: true })
      .click();
    expect((await customerResponsePromise).status()).toBe(200);
    await expect(customerTimePanel.getByRole("status")).toContainText(
      "已接受店家提議的新時間",
    );
      ...
      .poll(async () =>
        prisma.orderEvent.count({
          where: {
            orderId: order.id,
            eventType: {
              in: ["FULFILLMENT_TIME_PROPOSED", "FULFILLMENT_TIME_ACCEPTED"],
            },
          },
        }),
      )
...
test("外送頁依瀏覽器語系顯示英文欄位", async ({ browser }) => {
  ...
  try {
    ...
    await expect(
      deliveryCartPanel.getByLabel("Delivery address"),
    ).toBeVisible();
    await expect(
      deliveryCartPanel.getByRole("heading", {
        name: "Details and confirmation",
        exact: true,
      }),
    ).toBeVisible();
  } finally {
