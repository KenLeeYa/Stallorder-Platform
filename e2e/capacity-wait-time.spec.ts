...
test.describe.serial("產能與等候時間", () => {
        ...
        prisma.stallCapacitySettings.update({
          ...
          data: {
            ...
            warningUtilizationPercent:
              originalCapacity.warningUtilizationPercent,
            ...
            acknowledgmentThresholdMinutes:
              originalCapacity.acknowledgmentThresholdMinutes,
  ...
  test("商家可檢視完整容量設定與商品規則", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "產能與等候時間", exact: true }),
    ).toBeVisible();
    await expect(page.getByText("即時負載", { exact: true })).toBeVisible();
    await expect(
      page.getByText("等候時間與容量門檻", { exact: true }),
    ).toBeVisible();
  ...
  test("店員可手動暫停及恢復公開接單", async ({ page }) => {
    ...
    const panel = page
      .locator("details")
      .filter({ hasText: "產能與等候時間" })
      .first();
    await expect(panel).toBeVisible();
    await panel.locator("summary").click();
    await panel.getByLabel("操作原因").fill("E2E 現場人力調整");

    const pauseButton = panel.getByRole("button", {
      name: "暫停接單",
      exact: true,
    });
    ...
    await expect
      .poll(async () => (await loadCapacity()).pauseSource)
      .toBe("MANUAL");
    await expect
      .poll(async () => (await loadStall()).orderingState)
      .toBe("PAUSED");
  ...
  test("廚房角色不能存取容量控制或商家設定", async ({ page }) => {
    ...
    const capacityStatus = await page.evaluate(
      async (slug) =>
        (
          await fetch(`/api/stalls/${encodeURIComponent(slug)}/capacity`, {
            cache: "no-store",
          })
        ).status,
      stallSlug,
  ...
  test("高等候時間須由顧客確認後才能送出", async ({ page }) => {
    ...
    await expect(
      page
        ...
        .getByText("目前預估等候約 35 分鐘", { exact: true }),
    ).toBeVisible();
