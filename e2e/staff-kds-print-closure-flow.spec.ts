...
test.describe("單店員 KDS／列印分流與公休公告", () => {
      ...
      await Promise.all([
        ...
        prisma.product.findFirstOrThrow({
          where: {
            ...
            isActive: true,
            stallProducts: {
              some: { stallId, isEnabled: true, isSoldOut: false },
            },
          },
  ...
  test.afterAll(async () => {
    try {
      if (createdClosureIds.length > 0) {
        await prisma.stallSpecialClosure.deleteMany({
          where: { id: { in: createdClosureIds } },
        });
      }
  ...
  test("KDS 與列印皆關閉時，店員結帳一次即完成且不建立隱藏工作", async ({
    browser,
  }, testInfo) => {
    ...
    try {
      ...
      await expect(
        ownerPage.getByRole("link", { name: "廚房 KDS", exact: true }),
      ).toBeVisible();
      await expect(
        ownerPage.getByRole("link", { name: "KDS 工作站", exact: true }),
      ).toHaveCount(0);
      await expect(
        ownerPage.getByRole("link", { name: "KDS 設定", exact: true }),
      ).toHaveCount(0);

      await ownerPage.goto(`/staff/${stallSlug}`);
      await dismissStaffStartReminder(ownerPage);
      const visibleHeader = ownerPage
        .locator('[data-testid="staff-sticky-header"]:visible')
        .last();
      ...
      for (const viewport of [
        ...
        const header = ownerPage
          .locator('[data-testid="staff-sticky-header"]:visible')
          .last();
        const functions = header.locator(
          '[data-testid="staff-function-grid"]:visible',
        );
        ...
        const hitTargets = await functions
          .locator("button, a")
      ...
      await expect(
        ownerPage.getByRole("heading", { name: "找不到此頁面", exact: true }),
      ).toBeVisible();
    } finally {
    ...
    expect(await prisma.printJob.count({ where: { orderId: order.id } })).toBe(
      0,
    );
    expect(
      await prisma.orderProductionTask.count({ where: { orderId: order.id } }),
    ).toBe(0);
  ...
  test("KDS 關閉時，Menu 外帶單由店員完成並自動通知顧客可取餐", async ({
    browser,
  }) => {
    ...
    try {
      ...
      await expect(
        customerPage.getByText("攤位已確認", { exact: true }).first(),
      ).toBeVisible();
      ...
      const readyDialog = customerPage.getByRole("dialog", {
        name: "餐點已可取餐",
      });
      await expect(readyDialog).toBeVisible({ timeout: 20_000 });
      await expect(
        readyDialog.getByTestId("pickup-ready-dialog-code"),
      ).toHaveText(order.pickupCode);
    } finally {
  ...
  test("KDS 關閉但列印開啟時，確認即排入列印且收款後成功自動結單", async ({
    ...
    temporarilyDisabledPrinterIds.push(
      ...competingPrinters.map((printer) => printer.id),
    );
    ...
    try {
      ...
      await staffPage.addInitScript(() => {
        window.print = () => {
          ...
          window.sessionStorage.setItem(
            key,
            String(Number(window.sessionStorage.getItem(key) ?? "0") + 1),
          );
        };
      });
      await login(
      ...
      expect(pending.printJobs[0]).toMatchObject({
        status: "PENDING",
        printerId: createdPrinterId,
      });
      ...
      const startPrint = printJob.getByRole("button", {
        name: "開始列印",
        exact: true,
      });
      ...
      await expect
        .poll(() =>
          staffPage.evaluate(() =>
            window.sessionStorage.getItem("stallorder-qa-print-count"),
          ),
        )
        .toBe("1");

      const success = printJob.getByRole("button", {
        name: "成功",
        exact: true,
      });
      ...
      await expect
        .poll(async () => {
          ...
          return {
            ...
            printStatus: state.printJobs[0]?.status,
          };
        })
        .toEqual({
          status: "COMPLETED",
      ...
      await staffPage.screenshot({
        path: testInfo.outputPath("print-success-auto-complete.png"),
        fullPage: true,
      });
      ...
      await dismissStaffStartReminder(staffPage);
      await expect(
        staffPage.getByRole("article").filter({ hasText: order.customerName }),
      ).toHaveCount(0);
    } finally {
      await staffContext.close();
    }
  ...
  test("單日公休同步顯示於公開 Menu 與 QR，且顧客無法下單", async ({
    browser,
  }, testInfo) => {
    ...
    try {
      ...
      await ownerPage
        ...
        .click();
      const closureEditor = ownerPage.getByRole("dialog", {
        name: "特殊營業時間／店休",
      });
      await closureEditor.getByLabel("開始日期").fill(today);
      await closureEditor.getByLabel("結束日期").fill(today);
      await closureEditor
        .getByRole("button", { name: /^關閉 僅限所選日期/u })
        .click();
      await closureEditor.getByLabel("公告標題").fill(closureTitle);
      await closureEditor.getByLabel("補充說明（選填）").fill(closureMessage);
      ...
      const closureId = createPayload.closures.find(
        (closure) => closure.title === closureTitle,
      )?.id;
      ...
      try {
        ...
        await expect(customerPage.getByRole("main")).toContainText(
          product.name,
        );
        ...
        const qrProduct = qrMain
          .getByRole("article")
          .filter({ hasText: product.name });
        await expect(
          qrProductSelectionControl(qrProduct, product.name),
        ).toBeDisabled();
        await expect(qrMain.getByTestId("qr-mobile-cart-summary")).toHaveCount(
          0,
        );
        await expect(customerPage.locator("[data-nextjs-dialog]")).toHaveCount(
          0,
        );
        await customerPage.screenshot({
          path: testInfo.outputPath("qr-special-closure-mobile.png"),
          fullPage: true,
        });
      } finally {
      ...
      const closureArticle = ownerPage
        .getByRole("article")
        .filter({ hasText: closureTitle });
      await closureArticle.getByRole("button").click();
      const editEditor = ownerPage.getByRole("dialog", {
        name: "特殊營業時間／店休",
      });
      await editEditor
        .getByRole("button", { name: "刪除此設定", exact: true })
        .click();
...
function waitForPrintCommand(page: Page, operation: string) {
  return page.waitForResponse(
    (response) =>
      new URL(response.url()).pathname.endsWith(
        `/api/stalls/${stallSlug}/print-jobs`,
      ) &&
      response.request().method() === "POST" &&
...
function waitForClosureCommand(page: Page, operation: string) {
  return page.waitForResponse(
    (response) =>
      new URL(response.url()).pathname.endsWith(
        `/api/merchant/stalls/${stallId}/special-closures`,
      ) &&
      response.request().method() === "PATCH" &&
      requestOperation(response) === operation,
  );
...
function requestOperation(response: Response) {
  try {
    return (response.request().postDataJSON() as { operation?: string } | null)
      ?.operation;
  } catch {
    return undefined;
  }
