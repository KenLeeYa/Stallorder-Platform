...
const phase = process.env.PERFORMANCE_PHASE?.trim() || "baseline";
const sourceRevision = process.env.PERFORMANCE_SOURCE_REVISION?.trim() || null;
const functionRegion = process.env.PERFORMANCE_FUNCTION_REGION?.trim() || null;
const jsonOutputPath = resolve(process.env.PERFORMANCE_JSON_OUTPUT ?? "performance-results/baseline.json");
...
const routes = [
  ...
  ...(staffPath
    ...
    : []),
  ...(kitchenPath
    ? [{ label: "/kitchen（實際路由）", path: kitchenPath, budget: { totalP75Ms: 1_000 } }]
    : []),
  { label: "/merchant/dashboard", path: "/merchant/dashboard", budget: { totalP75Ms: 1_500 } },
  ...(reportsPath
    ? [{ label: "/merchant/reports（實際路由）", path: reportsPath }]
    : []),
];
...
const result = {
  ...
  methodology: {
    ...
    networkTiming: "每條路由使用獨立 keep-alive agent；第一筆通常可量到 DNS、TCP 與 TLS，重用連線時保留為 null。",
  },
  routes: [],
  warnings: [],
};
...
async function measureHttpRequest({
  ...
  try {
    return await new Promise((resolveRequest, rejectRequest) => {
      ...
      const request = (url.protocol === "https:" ? httpsRequest : httpRequest)(url, {
        ...
        response.once("end", () => {
          ...
          const contentType = singleHeaderValue(response.headers["content-type"]) ?? "";
          const connectedAt = timestamps.secureConnectAt ?? timestamps.connectAt;
          resolveRequest({
            kind: coldLike ? "cold_like" : "warm",
            status: response.statusCode ?? null,
            dnsMs: durationBetween(timestamps.socketAssignedAt, timestamps.lookupAt),
            tcpMs: durationBetween(timestamps.lookupAt ?? timestamps.socketAssignedAt, timestamps.connectAt),
            tlsMs: durationBetween(timestamps.connectAt, timestamps.secureConnectAt),
            connectionMs: durationBetween(timestamps.socketAssignedAt, connectedAt),
            ttfbMs: round(headersAt - startedAt),
            totalMs: round(completedAt - startedAt),
            responseBytes,
            htmlBytes: contentType.includes("text/html") ? responseBytes : 0,
            redirectLocationPresent: response.headers.location !== undefined,
            serverTiming: parseServerTiming(singleHeaderValue(response.headers["server-timing"])),
            cacheHeaders: {
              cacheControl: singleHeaderValue(response.headers["cache-control"]),
              vercelCdnCacheControl: singleHeaderValue(response.headers["vercel-cdn-cache-control"]),
              cdnCacheControl: singleHeaderValue(response.headers["cdn-cache-control"]),
              age: singleHeaderValue(response.headers.age),
              vercelCache: singleHeaderValue(response.headers["x-vercel-cache"]),
              vary: singleHeaderValue(response.headers.vary),
              setCookiePresent: response.headers["set-cookie"] !== undefined,
            },
            vercelCache: singleHeaderValue(response.headers["x-vercel-cache"]),
            vercelEdgePop: parseVercelEdgePop(singleHeaderValue(response.headers["x-vercel-id"])),
          });
        });
      });

      request.once("socket", (socket) => {
        timestamps.socketAssignedAt = performance.now();
        if (!socket.connecting) return;
        socket.once("lookup", () => {
          timestamps.lookupAt = performance.now();
        });
        socket.once("connect", () => {
          timestamps.connectAt = performance.now();
        });
        socket.once("secureConnect", () => {
          timestamps.secureConnectAt = performance.now();
        });
      });
    ...
    return {
      kind: coldLike ? "cold_like" : "warm",
      error: error instanceof Error ? error.message : "RequestError",
    };
  }
}
...
function summarizeSamples(samples) {
  ...
  return {
    ...
    dnsMs: summarizeNumbers(successful.map((sample) => sample.dnsMs)),
    connectionMs: summarizeNumbers(successful.map((sample) => sample.connectionMs)),
    tcpMs: summarizeNumbers(successful.map((sample) => sample.tcpMs)),
    tlsMs: summarizeNumbers(successful.map((sample) => sample.tlsMs)),
    ...
    cacheStatuses: [...new Set(successful.map((sample) => sample.vercelCache).filter(Boolean))],
...
function renderMarkdown(measurement) {
  const lines = [
    ...
    `- 階段：${measurement.phase}`,
    `- 版本：${measurement.sourceRevision ?? "未提供"}`,
    `- Function region：${measurement.functionRegion ?? "未提供"}`,
  ...
  for (const route of measurement.routes) {
    ...
    lines.push([
      `| ${route.route}`,
      route.httpStatus ?? "-",
      formatMs(route.coldLike?.dnsMs),
      formatMs(route.coldLike?.tcpMs),
      formatMs(route.coldLike?.tlsMs),
      ...
      route.warm?.cacheStatuses?.join(", ") || route.coldLike?.vercelCache || "-",
  ...
  for (const route of measurement.routes) {
    if (route.status !== "measured") continue;
    const headers = route.coldLike?.cacheHeaders ?? {};
    lines.push(`| ${route.route} | ${formatHeader(headers.cacheControl)} | ${formatHeader(headers.vercelCdnCacheControl)} | ${formatHeader(headers.cdnCacheControl)} | ${formatHeader(headers.age)} | ${formatHeader(headers.vary)} | ${headers.setCookiePresent ? "有" : "無"} |`);
  }
