...
- Status: Cloudflare Access architecture implemented in source; protected remote Plan/Apply not yet run; no DNS, Access, or Vercel mutation applied
- Date: 2026-09-01
...
| Cloudflare Access | Not enabled for the account; the current DNS-capable token cannot read or create Access resources. A least-privilege Access-capable token and one-time Zero Trust activation are prerequisites. |
| Vercel plan and project state | Pro plan; no `stallorder-dr` project; Standard protection is available for generated deployments, while paid `All Deployments` protection was rejected by the provider. |
...
3. `dr.qidaigo.com` may be provisioned only after an exact DR deployment exists with DR-specific bindings, `AUTH_PROJECT_CODE=DR`, active-backend/epoch checks, writer fencing, and access protection. Its Cloudflare DNS record must be proxied, its Access application must allow current Cloudflare account members, and the DR origin must validate the Access JWT signature, issuer, and application audience.
...
7. The separate Vercel project uses Standard protection (`all_except_custom_domains`) for its generated deployment URL. It does not purchase Advanced Deployment Protection. The custom domain trust boundary is Cloudflare Access plus origin JWT validation, not a Vercel project setting.
8. Automated Apply QA creates a one-hour Cloudflare Access service token and an application-owned Service Auth policy. Credentials remain only in process memory, are never written to evidence, and the token and policy are deleted and read back as absent after verification. Normal operator login uses the Cloudflare identity provider restricted to current account members.
9. The protected `/api/health/dr/operator` endpoint is disabled by default and returns `READY` only when the runtime, Supabase project ref/URLs, database identity, promotion epoch, `READ_ONLY_STANDBY` role, disabled writes, and writer-fence enforcement all match. Apply separately checks DR Auth health, Storage health, and that every repository Edge Function is active in the DR project.
...
The immutable Plan is created by `.github/workflows/production-dr-operator-entry.yml` and must identify the exact Cloudflare account/zone, Access team domain and account-member-restricted IdP, Vercel team/project, source commit/tree, DR runtime epoch, DNS before-state, Vercel domain before-state, access-protection mode, rollback operations, and these checks:
...
- no `dr.qidaigo.com` Access application, DNS record, Vercel binding/project, or colliding QA service token already exists;
- the Cloudflare token has least-privilege Access application/policy, service-token, organization/IdP read, and zone DNS capabilities; provider terms or Zero Trust activation are never accepted implicitly.

Apply order:

1. Create the one-hour QA service token and the self-hosted Cloudflare Access application with separate account-member Allow and Service Auth policies; read back the exact policy selectors.
2. Create the separate `stallorder-dr` project with Vercel Standard protection; read it back before deployment.
...
5. Add the `dr.qidaigo.com` Vercel binding, then explicitly promote the exact staged deployment created with `--prod --skip-domain`. The target project must contain only the Plan-bound DR hostname before promotion.
...
10. Verify `staging` is retired, `dr` is protected, and `app` remains healthy.

Rollback restores the recorded DNS/domain before-state and removes only the exact Vercel project, Cloudflare DNS record, Access application, and QA service-token IDs created by that Apply. It never deletes by name alone and must read the restored state back. It must not change database writer state. A failed DR probe stops before any `app.qidaigo.com` cutover.
