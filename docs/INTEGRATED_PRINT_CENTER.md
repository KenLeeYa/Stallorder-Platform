...
- Order completion uses the first primary job and its same-document manual reprints, including a reprint of a reprint. Confirmed success anywhere in that family satisfies the print requirement without rewriting failed/cancelled history. An unrelated receipt, routing copy or reprint of a routing copy cannot substitute for the primary ticket. A later failed spare copy cannot revoke confirmed output.
- The Staff board, completion API and automatic post-print completion share that status calculation. A paid counter order still at CONFIRMED/PREPARING/PACKING remains available for the operator to complete after recovery; only an eligible paid READY counter order auto-completes. QR payment/pickup, KDS and external-order gates remain independent. See [print recovery regression](PRINT_COMPLETION_RECOVERY_20260911.md).
