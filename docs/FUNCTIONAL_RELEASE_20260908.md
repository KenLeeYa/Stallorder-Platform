...
新鮮隔離環境末段 23 項瀏覽器流程全數通過，沒有略過或 flaky。註記管理首次載入的 strict locator 失敗亦在本機重現：trace 顯示第二個入口位於 Next.js 的 hidden streaming segment `S:0`，使用者可見入口只有一個。測試先等待入口數量回到一個，再保留原有可見性、同步、排序及刪除防護斷言；整份商品註記七項流程重新執行全部通過。

整套 CI 留存資料另揭露看板測試取用任意首張訂單的相依性：當首張為空品項測試單時，原測試複製不到品項，因此誤報無法捲動。已用隔離空品項前置資料重現；改為選取自己建立的 25 品項訂單，以實際資料驗證三區獨立捲動，保留預設選單、滿版、各區排序及不溢位斷言。完整店員響應式／列印三項流程、lint 與 typecheck 通過，應用版型不變。

Staging 合併後的 CI 在註記修改後重載，再次攔到 hidden streaming segment 的搜尋欄重複選取。將同類商品／註記入口及 inline board 限定至可見元素，保留唯一可見入口、焦點、手機隱藏、價格及群組持久化斷言；不以重試通過取代修正，也未調整 failOnFlakyTests。保留多組織範例時，測試會正常選擇指定商戶再驗證目的頁。商品、套餐、單一註記、註記群組及響應式共 18 項 production server 流程全數通過，lint、typecheck 與 build 通過。

原先失敗的註記編輯／重載案例另以新瀏覽器 context 連跑三次，全數通過，無略過或 flaky。
