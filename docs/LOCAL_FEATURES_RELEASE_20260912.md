...
- `40a9820` 的隔離 Preview `34675885114` 與安全掃描通過；CI `34675885086` 前六分片通過，第七分片僅舊版桌面工具列測試仍要求 `overflowY: visible`。新版需保留橫向捲動讓平板的印表機／通知入口可達，已改驗證橫向可捲動、沒有垂直或全頁水平溢出，保留按鈕開窗與內用欄位對齊斷言；本機乾淨資料庫回歸通過。其餘店員收款、列印失敗恢復、外送改時間及手機點餐流程在該 CI 已通過，仍需新 SHA 全套 CI。
