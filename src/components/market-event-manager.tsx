  ...
  async function mutate(command: Record<string, unknown>, successMessage: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setHasError(true);
        setMessage(typeof payload.error === "string" ? payload.error : "目前無法更新市集活動。");
        return;
      }
      setData(payload);
    ...
    <div ref={managerRef} className="space-y-8">
      <section aria-labelledby="event-form-title">
        ...
        <form noValidate onSubmit={(event) => void submit(event)} className="mt-5 grid gap-4 sm:grid-cols-2">
          <Field label="活動名稱" field="name" error={fieldErrors.name}><input type="text" required maxLength={150} value={form.name} {...validationProps("name", fieldErrors.name)} onChange={(event) => { clearFieldError("name"); setForm({ ...form, name: event.target.value }); }} className={inputClass} /></Field>
          <Field label="活動代稱" field="slug" error={fieldErrors.slug}><input type="text" required maxLength={100} pattern="[a-z0-9]+(?:-[a-z0-9]+)*" value={form.slug} {...validationProps("slug", fieldErrors.slug)} onChange={(event) => { clearFieldError("slug"); setForm({ ...form, slug: event.target.value.toLowerCase() }); }} className={inputClass} placeholder="weekend-market" /></Field>
          <Field label="場地名稱" field="venueName" error={fieldErrors.venueName}><input type="text" required maxLength={150} value={form.venueName} {...validationProps("venueName", fieldErrors.venueName)} onChange={(event) => { clearFieldError("venueName"); setForm({ ...form, venueName: event.target.value }); }} className={inputClass} /></Field>
          <Field label="地址" field="address" error={fieldErrors.address}><input type="text" required maxLength={300} value={form.address} {...validationProps("address", fieldErrors.address)} onChange={(event) => { clearFieldError("address"); setForm({ ...form, address: event.target.value }); }} className={inputClass} /></Field>
          <Field label="開始時間" field="startsAt" error={fieldErrors.startsAt}><input required type="datetime-local" value={form.startsAt} {...validationProps("startsAt", fieldErrors.startsAt)} onChange={(event) => { clearFieldError("startsAt"); clearFieldError("endsAt"); setForm({ ...form, startsAt: event.target.value }); }} className={inputClass} /></Field>
          <Field label="結束時間" field="endsAt" error={fieldErrors.endsAt}><input required type="datetime-local" value={form.endsAt} {...validationProps("endsAt", fieldErrors.endsAt)} onChange={(event) => { clearFieldError("endsAt"); setForm({ ...form, endsAt: event.target.value }); }} className={inputClass} /></Field>
          <Field label="緯度（選填）" field="latitude" error={fieldErrors.latitude}><input type="number" inputMode="decimal" min={-90} max={90} step="any" value={form.latitude} {...validationProps("latitude", fieldErrors.latitude)} onChange={(event) => { clearFieldError("latitude"); clearFieldError("longitude"); setForm({ ...form, latitude: event.target.value }); }} className={inputClass} /></Field>
          <Field label="經度（選填）" field="longitude" error={fieldErrors.longitude}><input type="number" inputMode="decimal" min={-180} max={180} step="any" value={form.longitude} {...validationProps("longitude", fieldErrors.longitude)} onChange={(event) => { clearFieldError("longitude"); clearFieldError("latitude"); setForm({ ...form, longitude: event.target.value }); }} className={inputClass} /></Field>
          <Field label="主辦單位（選填）" field="organizer" error={fieldErrors.organizer}><input type="text" maxLength={150} value={form.organizer} {...validationProps("organizer", fieldErrors.organizer)} onChange={(event) => { clearFieldError("organizer"); setForm({ ...form, organizer: event.target.value }); }} className={inputClass} /></Field>
          <Field label="公開網址（選填）" field="publicUrl" error={fieldErrors.publicUrl}><input type="url" maxLength={500} value={form.publicUrl} {...validationProps("publicUrl", fieldErrors.publicUrl)} onChange={(event) => { clearFieldError("publicUrl"); setForm({ ...form, publicUrl: event.target.value }); }} className={inputClass} placeholder="https://" /></Field>
          <Field label="活動說明（選填）" field="description" error={fieldErrors.description} wide><textarea maxLength={1000} value={form.description} {...validationProps("description", fieldErrors.description)} onChange={(event) => { clearFieldError("description"); setForm({ ...form, description: event.target.value }); }} className="mt-1 min-h-24 w-full rounded-md border border-stone-300 p-3 text-sm" /></Field>
          <label className="flex min-h-11 flex-wrap items-center gap-3 text-sm font-semibold"><input type="checkbox" checked={form.isPublic} {...validationProps("isPublic", fieldErrors.isPublic)} onChange={(event) => { clearFieldError("isPublic"); setForm({ ...form, isPublic: event.target.checked }); }} />允許顧客在公開行程頁看到活動資訊{fieldErrors.isPublic ? <span id={fieldErrorId("isPublic")} role="alert" className="w-full text-xs text-red-700">{fieldErrors.isPublic}</span> : null}</label>
