    ...
    <div className="space-y-8">
      <section aria-labelledby="event-form-title">
        ...
        <form onSubmit={(event) => void submit(event)} className="mt-5 grid gap-4 sm:grid-cols-2">
          <Field label="活動名稱"><input type="text" required maxLength={150} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className={inputClass} /></Field>
          <Field label="活動代稱"><input type="text" required maxLength={100} pattern="[a-z0-9]+(?:-[a-z0-9]+)*" value={form.slug} onChange={(event) => setForm({ ...form, slug: event.target.value.toLowerCase() })} className={inputClass} placeholder="weekend-market" /></Field>
          <Field label="場地名稱"><input type="text" required maxLength={150} value={form.venueName} onChange={(event) => setForm({ ...form, venueName: event.target.value })} className={inputClass} /></Field>
          ...
          <Field label="緯度（選填）"><input type="number" inputMode="decimal" min={-90} max={90} step="any" value={form.latitude} onChange={(event) => setForm({ ...form, latitude: event.target.value })} className={inputClass} /></Field>
          <Field label="經度（選填）"><input type="number" inputMode="decimal" min={-180} max={180} step="any" value={form.longitude} onChange={(event) => setForm({ ...form, longitude: event.target.value })} className={inputClass} /></Field>
          <Field label="主辦單位（選填）"><input type="text" maxLength={150} value={form.organizer} onChange={(event) => setForm({ ...form, organizer: event.target.value })} className={inputClass} /></Field>
