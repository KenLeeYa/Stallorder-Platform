...
export function CapacitySettingsForm({
  ...
  async function request(command: Record<string, unknown>) {
    ...
    if (!response.ok) {
      throw new CapacityFormError(payload.error ?? "無法更新容量設定。", payload.fieldErrors ?? {});
    }
    setData(payload);
    ...
    <div ref={formRef} className="space-y-7">
      ...
      <details className="border-b border-stone-200 pb-6" open>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <NumberField label="計算視窗（分鐘）" fieldKey="windowMinutes" error={fieldErrors.windowMinutes} min={5} max={120} value={settings.windowMinutes} onChange={(value) => updateNumber("windowMinutes", value)} />
          <NumberField label="每視窗訂單上限" fieldKey="maxOrdersPerWindow" error={fieldErrors.maxOrdersPerWindow} min={1} max={1000} value={settings.maxOrdersPerWindow} onChange={(value) => updateNumber("maxOrdersPerWindow", value)} />
          <NumberField label="每視窗品項上限" fieldKey="maxItemsPerWindow" error={fieldErrors.maxItemsPerWindow} min={1} max={5000} value={settings.maxItemsPerWindow} onChange={(value) => updateNumber("maxItemsPerWindow", value)} />
          <NumberField label="警示門檻（%）" fieldKey="warningUtilizationPercent" error={fieldErrors.warningUtilizationPercent} min={1} max={99} value={settings.warningUtilizationPercent} onChange={(value) => updateNumber("warningUtilizationPercent", value)} />
          <NumberField label="自動暫停門檻（%）" fieldKey="pauseUtilizationPercent" error={fieldErrors.pauseUtilizationPercent} min={2} max={200} value={settings.pauseUtilizationPercent} onChange={(value) => updateNumber("pauseUtilizationPercent", value)} />
          <NumberField label="基本製餐時間（分鐘）" fieldKey="defaultPrepMinutes" error={fieldErrors.defaultPrepMinutes} min={0} max={240} value={settings.defaultPrepMinutes} onChange={(value) => updateNumber("defaultPrepMinutes", value)} />
          <NumberField label="最短報價（分鐘）" fieldKey="minimumQuoteMinutes" error={fieldErrors.minimumQuoteMinutes} min={0} max={240} value={settings.minimumQuoteMinutes} onChange={(value) => updateNumber("minimumQuoteMinutes", value)} />
          <NumberField label="最長報價（分鐘）" fieldKey="maximumQuoteMinutes" error={fieldErrors.maximumQuoteMinutes} min={0} max={480} value={settings.maximumQuoteMinutes} onChange={(value) => updateNumber("maximumQuoteMinutes", value)} />
          <NumberField label="緩衝時間（分鐘）" fieldKey="quoteBufferMinutes" error={fieldErrors.quoteBufferMinutes} min={0} max={60} value={settings.quoteBufferMinutes} onChange={(value) => updateNumber("quoteBufferMinutes", value)} />
          <NumberField label="需顧客確認門檻（分鐘）" fieldKey="acknowledgmentThresholdMinutes" error={fieldErrors.acknowledgmentThresholdMinutes} min={1} max={480} value={settings.acknowledgmentThresholdMinutes} onChange={(value) => updateNumber("acknowledgmentThresholdMinutes", value)} />
        </div>
        <p className="mt-3 text-xs leading-5 text-stone-500">
          QR 顯示時間由基本製餐時間、緩衝時間、目前未完成訂單與品項數量、各商品權重／製餐時間、啟用工作站及近期實際完成速度自動計算；負載增加時會自動提高。
        </p>
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          ...
          <Toggle label="負載下降自動恢復" fieldKey="autoResumeEnabled" error={fieldErrors.autoResumeEnabled} checked={settings.autoResumeEnabled} disabled={!data.capabilities.automaticControl} onChange={(autoResumeEnabled) => { clearFieldError("autoResumeEnabled"); setSettings({ ...settings, autoResumeEnabled }); }} />
        </div>
      ...
      <details className="border-b border-stone-200 pb-6" open>
        ...
        <label className="mt-4 block text-sm font-medium">操作原因<input type="text" value={reason} maxLength={200} data-field-key="reason" aria-invalid={Boolean(fieldErrors.reason)} aria-describedby={fieldErrors.reason ? "capacity-reason-error" : undefined} onChange={(event) => { clearFieldError("reason"); setReason(event.target.value); }} placeholder="例如：人力不足、恢復正常" className="form-input mt-1" />{fieldErrors.reason ? <span id="capacity-reason-error" className="mt-1 block text-xs text-red-700">{fieldErrors.reason}</span> : null}</label>
        <div className="mt-4 grid gap-4 sm:grid-cols-[minmax(0,240px)_1fr] sm:items-end">
          <label className="text-sm font-medium">手動等候時間（分鐘）<input type="number" min={0} max={480} value={manualWait} data-field-key="minutes" aria-invalid={Boolean(fieldErrors.minutes)} aria-describedby={fieldErrors.minutes ? "capacity-minutes-error" : undefined} onChange={(event) => { clearFieldError("minutes"); setManualWait(event.target.value); }} placeholder="留空使用自動計算" className="form-input mt-1" />{fieldErrors.minutes ? <span id="capacity-minutes-error" className="mt-1 block text-xs text-red-700">{fieldErrors.minutes}</span> : null}</label>
        ...
        {!data.capabilities.productRules ? <p className="mt-3 text-sm text-stone-500">目前方案未包含各商品權重與時段數量上限。</p> : (
          <>
            <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
              <label className="text-sm font-medium lg:col-span-2">商品<select value={rule.productId} data-field-key="productId" aria-invalid={Boolean(fieldErrors.productId)} aria-describedby={fieldErrors.productId ? "capacity-product-error" : undefined} onChange={(event) => { clearFieldError("productId"); setRule({ ...rule, productId: event.target.value }); }} className="form-input mt-1"><option value="">請選擇</option>{data.products.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select>{fieldErrors.productId ? <span id="capacity-product-error" className="mt-1 block text-xs text-red-700">{fieldErrors.productId}</span> : null}</label>
              <TextNumberField label="產能權重" fieldKey="capacityWeight" error={fieldErrors.capacityWeight} min={0.1} max={100} step={0.1} value={rule.capacityWeight} onChange={(capacityWeight) => { clearFieldError("capacityWeight"); setRule({ ...rule, capacityWeight }); }} />
              <TextNumberField label="單品製餐時間（分鐘）" fieldKey="prepMinutes" error={fieldErrors.prepMinutes} min={0} max={240} value={rule.prepMinutes} onChange={(prepMinutes) => { clearFieldError("prepMinutes"); setRule({ ...rule, prepMinutes }); }} />
              <TextNumberField label="視窗數量上限" fieldKey="maxQuantityPerWindow" error={fieldErrors.maxQuantityPerWindow} min={1} max={5000} value={rule.maxQuantityPerWindow} placeholder="不限" onChange={(maxQuantityPerWindow) => { clearFieldError("maxQuantityPerWindow"); setRule({ ...rule, maxQuantityPerWindow }); }} />
            </div>
