...
function PrintTicket({ job, stallName, currency }: { job: PrintJobView | null; stallName: string; currency: string }) {
  if (!job) return null;
  return <article className="hidden print:block"><h1 className="text-2xl font-bold">{stallName}</h1><p className="mt-1">訂單 {job.order.orderNo} · {job.order.tableLabel ?? job.order.customerName}</p><ul className="my-4 divide-y border-y">{job.order.items.map((item) => <li key={item.id} className="py-2"><strong>{item.quantity} × {item.name}</strong>{item.noteOptions.length > 0 ? <div>{item.noteOptions.map((note) => `${note.groupName}：${note.optionName}`).join("、")}</div> : null}{item.note ? <div>備註：{item.note}</div> : null}</li>)}</ul><strong>合計 {formatMoney(job.order.total, currency)}</strong></article>;
}
