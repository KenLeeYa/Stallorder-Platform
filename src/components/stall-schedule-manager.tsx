...
export function StallScheduleManager({ stallId, initialData }: { stallId: string; initialData: Data }) {
  ...
  async function mutate(command: Record<string, unknown>, successMessage: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessage(payload.error ?? "目前無法更新出攤行程。");
        setHasError(true);
  ...
  async function copySchedule(scheduleId: string) {
    await mutate({ operation: "COPY_WEEKLY", scheduleId, weeks: Number(copyWeeks), reason }, `已建立未來 ${copyWeeks} 週行程。`);
  }
    ...
    <div ref={managerRef} className="space-y-10">
      <section aria-labelledby="schedule-form-title">
        ...
        <form noValidate onSubmit={(event) => void submit(event)} className="mt-5 grid gap-4 sm:grid-cols-2">
          <Field label="常用地點" field="locationId" error={fieldErrors.locationId}><select {...validationProps("locationId", fieldErrors.locationId)} value={form.locationId} onChange={(event) => { clearFieldError("locationId"); setForm({ ...form, locationId: event.target.value }); }} className={inputClass(fieldErrors.locationId)}><option value="">不指定</option>{data.locations.filter((location) => location.isActive).map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select></Field>
          {data.capabilities.eventSchedule ? <Field label="市集活動" field="marketEventId" error={fieldErrors.marketEventId}><select {...validationProps("marketEventId", fieldErrors.marketEventId)} value={form.marketEventId} onChange={(event) => { clearFieldError("marketEventId"); setForm({ ...form, marketEventId: event.target.value }); }} className={inputClass(fieldErrors.marketEventId)}><option value="">不指定</option>{data.events.map((event) => <option key={event.id} value={event.id}>{event.name} · {event.venueName}</option>)}</select></Field> : <div className="text-sm text-stone-500 sm:self-end">Pro 方案可建立跨攤位市集活動。</div>}
          <Field label="行程開始" field="startsAt" error={fieldErrors.startsAt}><input {...validationProps("startsAt", fieldErrors.startsAt)} type="datetime-local" required value={form.startsAt} onChange={(event) => { clearFieldError("startsAt"); setForm({ ...form, startsAt: event.target.value }); }} className={inputClass(fieldErrors.startsAt)} /></Field>
          <Field label="行程結束" field="endsAt" error={fieldErrors.endsAt}><input {...validationProps("endsAt", fieldErrors.endsAt)} type="datetime-local" required value={form.endsAt} onChange={(event) => { clearFieldError("endsAt"); setForm({ ...form, endsAt: event.target.value }); }} className={inputClass(fieldErrors.endsAt)} /></Field>
          <Field label="開放接單（留空同開始時間）" field="orderingOpensAt" error={fieldErrors.orderingOpensAt}><input {...validationProps("orderingOpensAt", fieldErrors.orderingOpensAt)} type="datetime-local" value={form.orderingOpensAt} onChange={(event) => { clearFieldError("orderingOpensAt"); setForm({ ...form, orderingOpensAt: event.target.value }); }} className={inputClass(fieldErrors.orderingOpensAt)} /></Field>
          <Field label="停止接單（留空同結束時間）" field="orderingClosesAt" error={fieldErrors.orderingClosesAt}><input {...validationProps("orderingClosesAt", fieldErrors.orderingClosesAt)} type="datetime-local" value={form.orderingClosesAt} onChange={(event) => { clearFieldError("orderingClosesAt"); setForm({ ...form, orderingClosesAt: event.target.value }); }} className={inputClass(fieldErrors.orderingClosesAt)} /></Field>
          <label className="block text-xs font-semibold text-stone-600 sm:col-span-2">公開臨時公告（選填）<textarea {...validationProps("specialNotice", fieldErrors.specialNotice)} maxLength={500} value={form.specialNotice} onChange={(event) => { clearFieldError("specialNotice"); setForm({ ...form, specialNotice: event.target.value }); }} className={`mt-1 min-h-20 w-full rounded-md border p-3 text-sm ${fieldErrors.specialNotice ? "border-red-500 bg-red-50" : "border-stone-300"}`} />{fieldErrors.specialNotice ? <FieldError field="specialNotice" error={fieldErrors.specialNotice} /> : null}</label>
          <label className="flex min-h-11 flex-wrap items-center gap-3 text-sm font-semibold"><input {...validationProps("autoOpenEnabled", fieldErrors.autoOpenEnabled)} type="checkbox" disabled={!data.capabilities.automaticOrdering} checked={form.autoOpenEnabled} onChange={(event) => { clearFieldError("autoOpenEnabled"); setForm({ ...form, autoOpenEnabled: event.target.checked }); }} />到時自動開放接單{fieldErrors.autoOpenEnabled ? <FieldError field="autoOpenEnabled" error={fieldErrors.autoOpenEnabled} /> : null}</label>
          <label className="flex min-h-11 flex-wrap items-center gap-3 text-sm font-semibold"><input {...validationProps("autoCloseEnabled", fieldErrors.autoCloseEnabled)} type="checkbox" disabled={!data.capabilities.automaticOrdering} checked={form.autoCloseEnabled} onChange={(event) => { clearFieldError("autoCloseEnabled"); setForm({ ...form, autoCloseEnabled: event.target.checked }); }} />到時自動停止接單{fieldErrors.autoCloseEnabled ? <FieldError field="autoCloseEnabled" error={fieldErrors.autoCloseEnabled} /> : null}</label>
      ...
      <section aria-labelledby="qr-schedule-title">
        ...
        <div className="mt-4 grid gap-4 rounded-md border border-stone-200 bg-white p-4 sm:grid-cols-3"><Field label="QR Code" field="qrCodeId" error={fieldErrors.qrCodeId}><select {...validationProps("qrCodeId", fieldErrors.qrCodeId)} value={qrCodeId} onChange={(event) => { clearFieldError("qrCodeId"); setQrCodeId(event.target.value); }} className={inputClass(fieldErrors.qrCodeId)}>{data.qrCodes.map((item) => <option key={item.id} value={item.id}>{item.label} · {item.state}</option>)}</select></Field><Field label="綁定行程" field="scheduleId" error={fieldErrors.scheduleId}><select {...validationProps("scheduleId", fieldErrors.scheduleId)} value={qrScheduleId} onChange={(event) => { clearFieldError("scheduleId"); setQrScheduleId(event.target.value); }} className={inputClass(fieldErrors.scheduleId)}><option value="">解除行程綁定</option>{data.schedules.filter((schedule) => ["SCHEDULED", "OPEN", "DELAYED"].includes(schedule.status)).map((schedule) => <option key={schedule.id} value={schedule.id}>{schedule.marketEvent?.name ?? schedule.location?.name ?? "行程"} · {formatShortDate(schedule.startsAt)}</option>)}</select></Field><Field label="點餐類型" field="fulfillmentType" error={fieldErrors.fulfillmentType}><select {...validationProps("fulfillmentType", fieldErrors.fulfillmentType)} disabled={Boolean(qr?.diningTableId) || !qrScheduleId} value={qr?.diningTableId ? "DINE_IN" : qrFulfillmentType} onChange={(event) => { clearFieldError("fulfillmentType"); setQrFulfillmentType(event.target.value as typeof qrFulfillmentType); }} className={inputClass(fieldErrors.fulfillmentType)}><option value="TAKEOUT">外帶</option><option value="DINE_IN">內用</option><option value="DELIVERY">外送</option></select></Field><div className="sm:col-span-3 sm:text-right"><button type="button" disabled={busy} onClick={() => void bindQr()} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-stone-900 px-4 text-sm font-semibold text-white disabled:opacity-50"><Link2 className="h-4 w-4" />套用 QR 綁定</button></div></div>
      </section>
    </div>
  );
}
