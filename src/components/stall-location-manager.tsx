  ...
  async function mutate(command: Record<string, unknown>, successMessage: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setMessage(payload.error ?? "目前無法更新地點。");
        setHasError(true);
    ...
    <div ref={managerRef} className="space-y-8">
      <section aria-labelledby="location-form-title">
        ...
        <form noValidate onSubmit={(event) => void submit(event)} className="mt-5 grid gap-4 sm:grid-cols-2">
          <Field label="地點名稱" field="name" error={fieldErrors.name}><input {...validationProps("name", fieldErrors.name)} type="text" required maxLength={100} value={form.name} onChange={(event) => { clearFieldError("name"); setForm({ ...form, name: event.target.value }); }} className={inputClass(fieldErrors.name)} /></Field>
          <Field label="地址" field="address" error={fieldErrors.address}><input {...validationProps("address", fieldErrors.address)} type="text" required maxLength={300} value={form.address} onChange={(event) => { clearFieldError("address"); setForm({ ...form, address: event.target.value }); }} className={inputClass(fieldErrors.address)} /></Field>
          <Field label="緯度（選填）" field="latitude" error={fieldErrors.latitude}><input {...validationProps("latitude", fieldErrors.latitude)} type="number" inputMode="decimal" min={-90} max={90} step="any" value={form.latitude} onChange={(event) => { clearFieldError("latitude"); setForm({ ...form, latitude: event.target.value }); }} className={inputClass(fieldErrors.latitude)} placeholder="25.056000" /></Field>
          ...
          <Field label="地圖網址（選填）" field="mapUrl" error={fieldErrors.mapUrl}><input {...validationProps("mapUrl", fieldErrors.mapUrl)} type="url" maxLength={500} value={form.mapUrl} onChange={(event) => { clearFieldError("mapUrl"); setForm({ ...form, mapUrl: event.target.value }); }} className={inputClass(fieldErrors.mapUrl)} placeholder="https://" /></Field>
          <Field label="到場說明（選填）" field="instructions" error={fieldErrors.instructions}><input {...validationProps("instructions", fieldErrors.instructions)} type="text" maxLength={500} value={form.instructions} onChange={(event) => { clearFieldError("instructions"); setForm({ ...form, instructions: event.target.value }); }} className={inputClass(fieldErrors.instructions)} /></Field>
