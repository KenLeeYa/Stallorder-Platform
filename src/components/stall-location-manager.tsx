    ...
    <div className="space-y-8">
      <section aria-labelledby="location-form-title">
        ...
        <form onSubmit={(event) => void submit(event)} className="mt-5 grid gap-4 sm:grid-cols-2">
          <Field label="地點名稱"><input required maxLength={100} value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className={inputClass} /></Field>
          ...
          <Field label="緯度（選填）"><input inputMode="decimal" value={form.latitude} onChange={(event) => setForm({ ...form, latitude: event.target.value })} className={inputClass} placeholder="25.056000" /></Field>
          <Field label="經度（選填）"><input inputMode="decimal" value={form.longitude} onChange={(event) => setForm({ ...form, longitude: event.target.value })} className={inputClass} placeholder="121.515000" /></Field>
          ...
          <Field label="到場說明（選填）"><input maxLength={500} value={form.instructions} onChange={(event) => setForm({ ...form, instructions: event.target.value })} className={inputClass} /></Field>
        ...
        <div className="mt-4 divide-y divide-stone-200 border-y border-stone-200">
          {data.locations.map((location) => (
            <article key={location.id} className="py-5">
              ...
              {confirmDeleteId === location.id ? <div className="mt-4 grid gap-3 border-l-2 border-red-500 pl-4 sm:grid-cols-[1fr_auto_auto] sm:items-end"><Field label="刪除原因"><input value={deleteReason} onChange={(event) => setDeleteReason(event.target.value)} className={inputClass} minLength={3} maxLength={300} /></Field><button type="button" disabled={busy || deleteReason.trim().length < 3} onClick={() => void mutate({ operation: "DELETE", locationId: location.id, reason: deleteReason }, "地點已刪除。") } className="min-h-10 rounded-md bg-red-700 px-3 text-sm font-semibold text-white disabled:opacity-50">確認刪除</button><button type="button" onClick={() => { setConfirmDeleteId(null); setDeleteReason(""); }} className="min-h-10 rounded-md border border-stone-300 px-3 text-sm font-semibold">保留</button></div> : null}
            </article>
          ))}
