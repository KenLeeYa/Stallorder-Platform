...
function formatStaffFulfillmentTime(value: string, locale: string) {
  return new Intl.DateTimeFormat(locale, { month: "numeric", day: "numeric", weekday: "short", hour: "2-digit", minute: "2-digit" }).format(new Date(value));
}
...
function staffStatusActionLabel(status: (typeof staffStatusOptions)[number]["value"], t: OperationsTranslator) {
  return status === "CONFIRMED" ? t("staff.status.confirmed") : status === "PREPARING" ? t("staff.action.startPreparing") : status === "PACKING" ? t("staff.status.packing") : status === "READY" ? t("staff.action.markReady") : status === "COMPLETED" ? t("staff.checkout.completeOrder") : t("staff.cancel.confirm");
}
...
function orderTimingSummary(order: StaffOrderDto, timing: FulfillmentProductionTiming | undefined, now: number, timeZone: string, locale: string, t: OperationsTranslator) {
...
function futureOrderTimeLabel(order: StaffOrderDto, timing: FulfillmentProductionTiming | undefined, timeZone: string, locale: string, t: OperationsTranslator) {
  const label = order.fulfillmentType === "DELIVERY" ? t("staff.timing.scheduledDelivery") : t("staff.timing.scheduledPickup");
...
function formatZonedDateTime(value: Date, timeZone: string, locale: string, includeDate = true) {
  return new Intl.DateTimeFormat(locale, { timeZone, ...(includeDate ? { month: "numeric", day: "numeric" } : {}), hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).format(value);
}
