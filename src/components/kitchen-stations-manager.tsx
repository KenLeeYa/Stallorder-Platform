...
export function KitchenStationsManager({ stallSlug, initialData }: { stallSlug: string; initialData: Data }) {
  ...
  const refresh = useCallback(async () => {
    ...
    if (!response.ok) throw new Error(t("kitchen.station.reloadFailed"));
    setData(payload);
  }, [stallSlug, t]);
  ...
  async function mutate(command: Record<string, unknown>, scope?: string) {
    ...
    try {
      ...
      setMessage(error instanceof Error ? error.message : t("kitchen.station.updateFailed"));
      return false;
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div ref={managerRef} className="mx-auto max-w-5xl">
      <div className="border-b border-stone-200 pb-5"><h2 className="text-2xl font-semibold">{t("kitchen.station.title")}</h2><p className="mt-2 text-sm text-stone-600">{t("kitchen.station.description")}</p></div>
...
function StationEditor({ station, categories, products, busy, fieldErrors, clearFieldError, mutate }: { station: Station; categories: Data["categories"]; products: Data["products"]; busy: boolean; fieldErrors: Record<string, string>; clearFieldError: (scope: string, field: string) => void; mutate: (command: Record<string, unknown>, scope?: string) => Promise<boolean> }) {
  ...
  return (
    <section className="rounded-md border border-stone-200 bg-white p-4 md:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-semibold">{station.name}</h3><p className="mt-1 font-mono text-xs text-stone-500">{station.code} · {t("kitchen.station.history", { count: station.taskCount })}</p></div><span className={`rounded-full px-2 py-1 text-xs font-semibold ${station.isActive ? "bg-emerald-100 text-emerald-800" : "bg-stone-200 text-stone-600"}`}>{station.isActive ? t("common.enabled") : t("common.disabled")}</span></div>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <Field label={t("kitchen.station.name")} error={fieldErrors[`${scope}:name`]} errorId={`${scope}-name-error`}><input aria-label={t("kitchen.station.name")} type="text" value={draft.name} maxLength={80} data-field-key={`${scope}:name`} aria-invalid={Boolean(fieldErrors[`${scope}:name`])} aria-describedby={fieldErrors[`${scope}:name`] ? `${scope}-name-error` : undefined} onChange={(event) => { clearFieldError(scope, "name"); setDraft({ ...draft, name: event.target.value }); }} className="form-input" /></Field>
        <Field label={t("kitchen.station.code")} error={fieldErrors[`${scope}:code`]} errorId={`${scope}-code-error`}><input aria-label={t("kitchen.station.code")} type="text" value={draft.code} disabled={station.code === "DEFAULT"} maxLength={32} data-field-key={`${scope}:code`} aria-invalid={Boolean(fieldErrors[`${scope}:code`])} aria-describedby={fieldErrors[`${scope}:code`] ? `${scope}-code-error` : undefined} onChange={(event) => { clearFieldError(scope, "code"); setDraft({ ...draft, code: event.target.value.toUpperCase() }); }} className="form-input font-mono disabled:bg-stone-100" /></Field>
        <Field label={t("kitchen.station.descriptionField")} error={fieldErrors[`${scope}:description`]} errorId={`${scope}-description-error`}><input aria-label={t("kitchen.station.descriptionField")} type="text" value={draft.description} maxLength={300} data-field-key={`${scope}:description`} aria-invalid={Boolean(fieldErrors[`${scope}:description`])} aria-describedby={fieldErrors[`${scope}:description`] ? `${scope}-description-error` : undefined} onChange={(event) => { clearFieldError(scope, "description"); setDraft({ ...draft, description: event.target.value }); }} className="form-input" /></Field>
        <Field label={t("kitchen.station.sort")} error={fieldErrors[`${scope}:sortOrder`]} errorId={`${scope}-sort-error`}><input aria-label={t("kitchen.station.sort")} type="number" min={0} max={10000} value={draft.sortOrder} data-field-key={`${scope}:sortOrder`} aria-invalid={Boolean(fieldErrors[`${scope}:sortOrder`])} aria-describedby={fieldErrors[`${scope}:sortOrder`] ? `${scope}-sort-error` : undefined} onChange={(event) => { clearFieldError(scope, "sortOrder"); setDraft({ ...draft, sortOrder: event.target.value === "" ? "" : Number(event.target.value) }); }} className="form-input" /></Field>
      </div>
      <label className="mt-3 flex min-h-11 items-center gap-3 text-sm font-medium"><input type="checkbox" checked={draft.isActive} onChange={(event) => setDraft({ ...draft, isActive: event.target.checked })} />{t("kitchen.station.enable")}</label>
      <div className="mt-3 flex flex-wrap gap-2"><button type="button" disabled={busy} onClick={() => void mutate({ operation: "UPDATE_STATION", stationId: station.id, ...draft, description: draft.description.trim() || null }, scope)} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-teal-800 px-4 text-sm font-semibold text-white disabled:opacity-50"><Save className="h-4 w-4" />{t("common.save")}</button>{station.code !== "DEFAULT" ? <button type="button" disabled={busy} onClick={() => { if (window.confirm(t("kitchen.station.deleteConfirm", { name: station.name }))) void mutate({ operation: "DELETE_STATION", stationId: station.id }); }} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-red-300 px-4 text-sm font-semibold text-red-700 disabled:opacity-50"><Trash2 className="h-4 w-4" />{t("common.delete")}</button> : null}</div>

      <div className="mt-6 border-t border-stone-200 pt-5"><h4 className="font-semibold">{t("kitchen.station.rules")}</h4><div className="mt-3 grid gap-3 sm:grid-cols-[140px_minmax(0,1fr)_auto]"><select value={targetType} onChange={(event) => { const next = event.target.value as "CATEGORY" | "PRODUCT"; setTargetType(next); setTargetId(next === "CATEGORY" ? categories[0]?.id ?? "" : products[0]?.id ?? ""); }} className="h-11 rounded-md border border-stone-300 bg-white px-3"><option value="CATEGORY">{t("kitchen.station.category")}</option><option value="PRODUCT">{t("kitchen.station.product")}</option></select><select value={targetId} onChange={(event) => setTargetId(event.target.value)} className="h-11 min-w-0 rounded-md border border-stone-300 bg-white px-3">{(targetType === "CATEGORY" ? categories : products).map((target) => <option key={target.id} value={target.id}>{target.name}</option>)}</select><button type="button" disabled={busy || !targetId} onClick={() => void createAssignment()} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-stone-300 px-4 text-sm font-semibold disabled:opacity-50"><Plus className="h-4 w-4" />{t("kitchen.station.assign")}</button></div>
        <div className="mt-3 divide-y divide-stone-100 border-y border-stone-100">{station.assignments.map((assignment) => <div key={assignment.id} className="flex min-h-12 items-center justify-between gap-3 py-2 text-sm"><span><strong>{assignment.product ? t("kitchen.station.assignmentProduct") : t("kitchen.station.assignmentCategory")}</strong> · {assignment.product?.name ?? assignment.category?.name}</span><button type="button" title={t("kitchen.station.deleteAssignment")} disabled={busy} onClick={() => void mutate({ operation: "DELETE_ASSIGNMENT", assignmentId: assignment.id })} className="grid h-9 w-9 place-items-center rounded-md text-red-700 hover:bg-red-50 disabled:opacity-50"><Trash2 className="h-4 w-4" /></button></div>)}</div>
      </div>
    </section>
  );
}
