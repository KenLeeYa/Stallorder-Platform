...
export function StallTemplateCopyManager({ stallId, sourceStalls }: { stallId: string; sourceStalls: Array<{ id: string; name: string }> }) {
  ...
  return <section aria-labelledby="stall-template-heading" data-settings-section data-settings-scope="stall-template" data-settings-search="多攤位範本 複製 付款 折扣 店員外送 預約 抽抽樂 商品供應 營業時間 差異預覽" className="border-b border-stone-200">
    <div className="flex min-h-14 items-center gap-3 py-3 text-left">
      ...
      <div className="min-w-0 flex-1">
        <h2 id="stall-template-heading" className="text-lg font-semibold">多攤位範本</h2>
        <p className="mt-1 text-sm text-stone-600">先顯示差異，再選擇要覆蓋的設定。</p>
      </div>
    </div>
    <div className="pb-6">
