...
import { Minus, Package, Plus, Send, ShoppingCart, Truck, Utensils, X } from "lucide-react";
...
export function StaffOrderComposer({
  ...
  async function submit() {
    ...
    if (fulfillmentType === "DELIVERY" && (customerPhone.trim().length < 6 || !deliveryAddress.trim())) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/45 p-3 print:hidden sm:p-6">
      <section role="dialog" aria-modal="true" aria-labelledby="staff-order-title" className="mx-auto min-h-full w-full max-w-6xl bg-white shadow-xl sm:min-h-0 sm:rounded-lg">
        <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-stone-200 bg-white px-4 py-4 sm:rounded-t-lg sm:px-6">
          <div>
          ...
          <button type="button" title="關閉店員點餐" disabled={busy} onClick={onClose} className="grid h-10 w-10 shrink-0 place-items-center rounded-md border border-stone-300"><X className="h-4 w-4" /></button>
        </header>
        ...
        <div className="grid lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="px-4 py-5 sm:px-6">
            ...
            <div className="mt-5 grid items-start gap-3 sm:grid-cols-2">
              <TextField className="mt-0" label="顧客名稱（選填）" value={customerName} maxLength={50} onChange={setCustomerName} />
              {(fulfillmentType === "TAKEOUT" || fulfillmentType === "DELIVERY") ? <TextField className="mt-0" label={fulfillmentType === "DELIVERY" ? "聯絡電話" : "聯絡電話（選填）"} value={customerPhone} maxLength={30} onChange={setCustomerPhone} /> : null}
              ...
              {fulfillmentType === "DELIVERY" ? <label className="text-xs font-semibold text-stone-600 sm:col-span-2">外送地址<textarea value={deliveryAddress} maxLength={300} onChange={(event) => setDeliveryAddress(event.target.value)} className="mt-1 min-h-20 w-full rounded-md border border-stone-300 px-3 py-2 text-sm" /></label> : null}
            </div>
          ...
          <aside className="border-t border-stone-200 bg-stone-50 px-4 py-5 lg:border-l lg:border-t-0 sm:px-6">
            ...
            <label className="mt-4 block text-xs font-semibold text-stone-600">訂單備註<textarea value={customerNote} maxLength={catalog.limits.maxNoteLength} onChange={(event) => setCustomerNote(event.target.value)} className="mt-1 min-h-20 w-full rounded-md border border-stone-300 bg-white px-3 py-2 text-sm" /></label>
...
function TextField({ className = "mt-3", label, value, maxLength, type = "text", inputMode, onChange }: { className?: string; label: string; value: string; maxLength: number; type?: string; inputMode?: React.HTMLAttributes<HTMLInputElement>["inputMode"]; onChange: (value: string) => void }) {
  return <label className={`${className} block text-xs font-semibold text-stone-600`}>{label}<input type={type} inputMode={inputMode} value={value} maxLength={maxLength} onChange={(event) => onChange(event.target.value)} className="mt-1 h-11 w-full rounded-md border border-stone-300 bg-white px-3 text-sm" /></label>;
}
