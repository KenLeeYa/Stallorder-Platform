...
export function LineIntegrationManager({
  ...
  async function save() {
    ...
    try {
      ...
      setMessage(error instanceof Error ? label(error.message) : m("無法儲存 LINE 整合設定。"));
      setHasError(true);
    } finally {
      setBusy(false);
    }
  ...
  async function disable() {
    ...
    try {
      ...
      setMessage(error instanceof Error ? label(error.message) : m("無法停用 LINE 整合。"));
      setHasError(true);
    } finally {
      setBusy(false);
    }
    ...
    <div ref={managerRef} className="space-y-8">
      <section className="border-y border-stone-200 py-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div><h2 className="text-lg font-semibold">{m("整合狀態")}</h2><p className="mt-1 text-sm text-stone-600">{data.status === "ACTIVE" ? m("已啟用") : data.status === "ERROR" ? m("需要檢查") : m("未啟用")}</p></div>
          <span className={`inline-flex min-h-9 items-center gap-2 rounded-md px-3 text-sm font-semibold ${data.status === "ACTIVE" ? "bg-emerald-50 text-emerald-800" : "bg-stone-100 text-stone-700"}`}><span className={`h-2.5 w-2.5 rounded-full ${data.status === "ACTIVE" ? "bg-emerald-600" : "bg-stone-400"}`} />{data.status === "ACTIVE" ? m("啟用") : m("停用")}</span>
        </div>
        {webhookUrl ? <div className="mt-5"><label className="text-sm font-medium" htmlFor="line-webhook-url">Webhook URL</label><div className="mt-2 flex gap-2"><input type="text" id="line-webhook-url" readOnly value={webhookUrl} className="min-h-11 min-w-0 flex-1 rounded-md border border-stone-300 bg-stone-50 px-3 text-sm" /><button type="button" title={m("複製 Webhook URL")} aria-label={m("複製 Webhook URL")} onClick={() => void copyWebhook()} className="grid h-11 w-11 shrink-0 place-items-center rounded-md border border-stone-300">{copied ? <Check className="h-4 w-4 text-emerald-700" /> : <Copy className="h-4 w-4" />}</button></div></div> : null}
      </section>
      ...
      <section className="border-y border-stone-200 py-5">
        <h2 className="text-lg font-semibold">{m("Vault 憑證")}</h2>
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
      ...
      <div className="flex flex-wrap gap-3 border-t border-stone-200 pt-5">
        <button type="button" onClick={() => void save()} disabled={busy} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-stone-950 px-4 text-sm font-semibold text-white disabled:opacity-40">{busy ? <LoaderCircle className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{m("儲存並輪替憑證")}</button>
