...
const capabilityOptions: ReadonlyArray<readonly [string, MerchantMessageKey]> = [
...
export function DeliveryConnectionRequestForm({ stallId }: { stallId: string }) {
    ...
    try {
      ...
      if (!response.ok) {
        const nextFieldErrors = Object.fromEntries(
          Object.entries(parseFieldErrors(result.fieldErrors)).map(([field, error]) => [field, label(error)]),
        );
        setFieldErrors(nextFieldErrors);
        focusFirstInvalidField(formRef.current, nextFieldErrors);
        setHasError(true);
        setMessage(typeof result.error === "string" ? label(result.error) : m("送出申請失敗。"));
        return;
      }
      setMessage(m("申請已送出，平台管理員將依合作資格進行審核。"));
      formElement.reset();
  ...
  return (
    <form ref={formRef} noValidate onSubmit={submit} className="grid gap-4 border-t border-stone-200 pt-5 md:grid-cols-2">
      <label className="text-sm font-medium">{m("外送平台")}
      ...
      <label className="text-sm font-medium">{m("聯絡人姓名")}
      ...
      <label className="text-sm font-medium">{m("聯絡電子郵件")}
      ...
      <label className="text-sm font-medium">{m("聯絡電話")}
      ...
      <label className="text-sm font-medium">{m("既有 Vendor Code（選填）")}
      ...
      <label className="text-sm font-medium">{m("既有 Chain Code（選填）")}
      ...
      <label className="text-sm font-medium md:col-span-2">{m("目前使用的點餐系統（選填）")}
      ...
      <fieldset className="md:col-span-2" aria-invalid={Boolean(fieldErrors.requestedCapabilities)} aria-describedby={fieldErrors.requestedCapabilities ? fieldErrorId("requestedCapabilities") : undefined}>
        <legend className="text-sm font-semibold">{m("預計使用功能")}</legend>
        <div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {capabilityOptions.map(([code, labelKey], index) => (
      ...
      <label className="text-sm font-medium md:col-span-2">{m("補充說明（選填）")}
      ...
      <div className="flex flex-wrap items-center gap-3 md:col-span-2">
        <button type="submit" disabled={pending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-teal-700 px-4 font-semibold text-white disabled:opacity-50">
          <Send className="h-4 w-4" />{pending ? m("送出中") : m("送出連線申請")}
        </button>
