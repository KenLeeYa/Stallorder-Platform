  ...
  async function save() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setHasError(true);
        setMessage(typeof result.error === "string" ? result.error : "無法儲存報表排程。");
        return;
      }
  ...
  return (
    <main ref={managerRef} className="mx-auto min-h-[calc(100vh-76px)] max-w-6xl px-4 py-7 md:px-8">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 border-b border-stone-200 pb-5">
        ...
        {!editing ? <button type="button" onClick={() => { setDraft(newDraft()); setEditingId(null); setFieldErrors({}); }} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-stone-900 px-4 text-sm font-semibold text-white"><Plus className="h-4 w-4" />新增排程</button> : null}
      </div>
      ...
      {draft ? <ScheduleEditor draft={draft} stalls={stalls} busy={busyId !== null} fieldErrors={fieldErrors} onChange={setDraft} onClearField={clearFieldError} onSave={() => void save()} onCancel={() => { setDraft(null); setEditingId(null); setFieldErrors({}); }} /> : null}
  ...
  return (
    <section className="border-b border-stone-200 py-6" aria-labelledby="editor-title">
      <div className="flex items-center justify-between gap-3">
        <h2 id="editor-title" className="text-lg font-semibold">排程設定</h2>
        <button type="button" title="關閉" onClick={onCancel} className="grid h-10 w-10 place-items-center rounded-md border border-stone-300"><X className="h-4 w-4" /></button>
      </div>
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <label className="text-sm font-medium">排程名稱
        ...
        <label className="text-sm font-medium">報告類型
          <select value={draft.reportType} data-field-key="reportType" aria-invalid={Boolean(fieldErrors.reportType)} aria-describedby={fieldErrors.reportType ? fieldErrorId("reportType") : undefined} onChange={(event) => { const reportType = event.target.value as ScheduledReportType; onClearField("reportType"); onClearField("dayOfWeek"); onChange({ ...draft, reportType, dayOfWeek: reportType === "WEEKLY_SALES" ? draft.dayOfWeek ?? 1 : null }); }} className="mt-1 h-11 w-full rounded-md border border-stone-300 bg-white px-3">
            {Object.entries(reportScheduleTypeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
          </select>
          <FieldError field="reportType" error={fieldErrors.reportType} />
        </label>
        <label className="text-sm font-medium">寄送時間
        ...
        {draft.reportType === "WEEKLY_SALES" ? (
          <label className="text-sm font-medium">寄送星期
        ...
        <label className="text-sm font-medium md:col-span-2">收件人 Email
        ...
        <fieldset tabIndex={-1} data-field-key="stallIds" aria-invalid={Boolean(fieldErrors.stallIds)} aria-describedby={fieldErrors.stallIds ? fieldErrorId("stallIds") : undefined} className="md:col-span-2">
          <legend className="text-sm font-medium">攤位範圍</legend>
        ...
        <label className="flex min-h-11 flex-wrap items-center gap-3 text-sm font-medium">
          <input type="checkbox" checked={draft.isEnabled} data-field-key="isEnabled" aria-invalid={Boolean(fieldErrors.isEnabled)} aria-describedby={fieldErrors.isEnabled ? fieldErrorId("isEnabled") : undefined} onChange={(event) => { onClearField("isEnabled"); onChange({ ...draft, isEnabled: event.target.checked }); }} />啟用此排程
          <FieldError field="isEnabled" error={fieldErrors.isEnabled} />
        </label>
      </div>
      <div className="mt-5 flex gap-2">
        <button type="button" disabled={busy} onClick={onSave} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-stone-900 px-4 text-sm font-semibold text-white disabled:opacity-50"><Save className="h-4 w-4" />儲存排程</button>
        <button type="button" disabled={busy} onClick={onCancel} className="min-h-11 rounded-md border border-stone-300 px-4 text-sm font-semibold disabled:opacity-50">取消</button>
      </div>
    </section>
  );
}
