  ...
  return (
    <div className="space-y-5">
      ...
      <form action={(formData) => operation({ operation: "ASSIGN_ORDER_PACKAGE", code: formData.get("code"), quantity: Number(formData.get("quantity")), reason: formData.get("reason") })} className="grid gap-3 border-t border-stone-200 pt-4 sm:grid-cols-2">
        ...
        <label className="text-sm font-medium sm:col-span-2">原因<input name="reason" minLength={2} maxLength={500} defaultValue="人工指派訂單包" className="mt-1 h-10 w-full rounded-md border border-stone-300 px-3" /></label>
        <button disabled={saving || orderPackages.length === 0} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-md bg-stone-900 px-4 text-sm font-semibold text-white disabled:opacity-50 sm:col-span-2"><PackagePlus className="h-4 w-4" />指派並加入帳單</button>
      </form>
      <form action={(formData) => operation({ operation: "REBUILD_USAGE", billingPeriod: `${formData.get("billingPeriod")}-01`, reason: formData.get("reason") })} className="grid gap-3 border-t border-stone-200 pt-4 sm:grid-cols-[180px_1fr_auto] sm:items-end"><label className="text-sm font-medium">計費月份<input name="billingPeriod" type="month" defaultValue={currentMonth()} required className="mt-1 h-10 w-full rounded-md border border-stone-300 px-3" /></label><label className="text-sm font-medium">原因<input name="reason" minLength={2} maxLength={500} defaultValue="人工用量對帳" className="mt-1 h-10 w-full rounded-md border border-stone-300 px-3" /></label><button disabled={saving} className="min-h-10 rounded-md border border-stone-300 px-4 text-sm font-semibold">重建用量</button></form>
...
export function AdminInvoiceVoidAction({ invoiceId }: { invoiceId: string }) {
  ...
  return <form action={voidInvoice} className="grid gap-3 sm:grid-cols-[1fr_auto] sm:items-end"><label className="text-sm font-medium">作廢原因<input name="reason" minLength={2} maxLength={500} defaultValue="帳務內容需重新建立" required className="mt-1 h-10 w-full rounded-md border border-stone-300 px-3" /></label><button disabled={saving} className="min-h-10 rounded-md border border-red-300 px-4 text-sm font-semibold text-red-800">{confirming ? "再次點擊確認作廢" : "作廢帳單"}</button>{message ? <p role="status" className="text-sm font-medium sm:col-span-2">{message}</p> : null}</form>;
}
