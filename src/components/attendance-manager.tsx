...
function riskLabel(code: string) { return ({ LOCATION_STALE: "定位資料過期", LOCATION_FUTURE: "定位時間異常", ACCURACY_TOO_POOR: "定位精度不足", OUTSIDE_GEOFENCE: "超出打卡範圍", GEOFENCE_BOUNDARY: "位於範圍邊界", ROTATING_CODE_INVALID: "動態驗證碼不正確", DEVICE_NOT_BOUND: "裝置登入綁定失效", IMPOSSIBLE_TRAVEL: "偵測到不合理位置跳躍", HIGH_TRAVEL_SPEED: "移動速度異常", LATE_CLOCK_IN: "遲到打卡", EARLY_CLOCK_OUT: "早退打卡", ALREADY_CLOCKED_IN: "已是上班狀態", NOT_CLOCKED_IN: "尚未上班打卡" } as Record<string, string>)[code] ?? code; }
