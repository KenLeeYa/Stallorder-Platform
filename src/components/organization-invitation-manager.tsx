...
const roleLabelKeys: Record<InvitationRole, MerchantMessageKey> = {
...
const statusLabelKeys: Record<string, MerchantMessageKey> = {
  PENDING: "等待接受",
  ACCEPTED: "已接受",
  EXPIRED: "已過期",
  REVOKED: "已撤銷",
...
export function OrganizationInvitationManager({
  ...
  async function createInvitation(formData: FormData) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setHasError(true);
        setMessage(typeof payload.error === "string" ? label(payload.error) : m("目前無法建立邀請。"));
        return;
      }
      ...
      setMessage(m("邀請已建立。基於安全考量，接受連結只會顯示這一次。"));
    } catch (caughtError) {
      setHasError(true);
      setMessage(caughtError instanceof Error ? label(caughtError.message) : m("目前無法建立邀請。"));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function revokeInvitation(invitation: Invitation) {
    if (!window.confirm(m("確定撤銷寄給 {email} 的邀請？撤銷後原連結將立即失效。", { email: invitation.email }))) return;
    setSaving(true);
    setMessage("");
    setHasError(false);
    try {
      ...
      if (!response.ok) throw new Error(typeof payload.error === "string" ? label(payload.error) : m("目前無法撤銷邀請。"));
      ...
      setHasError(true);
      setMessage(caughtError instanceof Error ? label(caughtError.message) : m("目前無法撤銷邀請。"));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <section className="py-7">
      <h2 className="text-lg font-semibold">{m("邀請新成員")}</h2>
      <form ref={formRef} noValidate action={createInvitation} className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_180px_auto]">
        ...
        <label className="text-sm font-medium">
          ...
          >
            ...
            {canManageOrganizationTeam ? <option value="ORGANIZATION_ADMIN">{m("組織管理員")}</option> : null}
            {canManageOrganizationTeam ? <option value="FINANCE_VIEWER">{m("財務檢視者")}</option> : null}
            {canManageOrganizationTeam ? <option value="STALL_MANAGER">{m("攤位經理")}</option> : null}
            <option value="STAFF">{m("店員")}</option>
            <option value="KITCHEN">{m("廚房")}</option>
          </select>
        ...
        <label className="text-sm font-medium">
          ...
          >
            {organizationRole ? <option value="">{m("全部攤位")}</option> : null}
      ...
      {acceptanceUrl ? (
        <div className="mt-5 border-y border-emerald-200 bg-emerald-50 py-4">
          <p className="text-sm font-semibold text-emerald-900">{m("一次性邀請連結")}</p>
          <div className="mt-2 flex gap-2">
            <input type="text"
              aria-label={m("一次性邀請連結")}
              readOnly
              value={acceptanceUrl}
            ...
            >
              ...
              <span className="sr-only">{m("複製邀請連結")}</span>
            </button>
          </div>
        </div>
      ) : null}
      ...
      <h3 className="mt-7 text-base font-semibold">{m("邀請紀錄")}</h3>
          ...
          <div key={invitation.id} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              ...
              <div className="mt-1 text-sm text-stone-500">
                {m("{role} · {scope}", {
                  role: m(roleLabelKeys[invitation.role]),
                  scope: invitation.stallId ? (stallNames.get(invitation.stallId) ?? m("已移除攤位")) : m("全部攤位"),
                })}
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="rounded-md bg-stone-100 px-2 py-1 text-xs font-semibold">
                {statusLabelKeys[invitation.status] ? m(statusLabelKeys[invitation.status]) : invitation.status}
              </span>
              {invitation.status === "PENDING" ? (
                ...
                >
                  ...
                  <span className="sr-only">{m("撤銷邀請")}</span>
                </button>
              ) : null}
            </div>
          </div>
      ...
      {invitations.length === 0 ? <p className="mt-4 text-sm text-stone-600">{m("尚無邀請紀錄。")}</p> : null}
    </section>
  );
}
