...
export function StaffOrderEditProductPicker({ product, currency, busy, onAdd }: {
  ...
  return <div className="mt-3 grid gap-3">
    ...
    <label className="text-sm font-medium">{t("staff.edit.itemNote")}<input type="text" maxLength={1000} value={note} disabled={busy} onChange={(event) => setNote(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-stone-300 px-3" /></label>
