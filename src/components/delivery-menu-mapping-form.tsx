...
export function DeliveryMenuMappingForm({
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setHasError(true);
        setMessage(typeof result.error === "string" ? label(result.error) : m("儲存對應失敗。"));
        return;
      }
      setMessage(m("對應已儲存。"));
  ...
  return (
    <form ref={formRef} noValidate onSubmit={submit} className="grid gap-4 border-t border-stone-200 pt-5 md:grid-cols-2">
      <label className="text-sm font-medium">{m("資料類型")}
        <select value={type} {...validationProps("internalEntityType", fieldErrors.internalEntityType)} onChange={(event) => { clearFieldError("internalEntityType"); clearFieldError("internalEntityId"); setType(event.target.value as MappingEntity["type"]); }} className="mt-1 h-11 w-full rounded-md border border-stone-300 bg-white px-3">
          <option value="CATEGORY">{m("商品分類")}</option>
          <option value="PRODUCT">{m("商品")}</option>
          <option value="MODIFIER_GROUP">{m("註記群組")}</option>
          <option value="MODIFIER_ITEM">{m("註記項目")}</option>
        </select>
        <FieldError field="internalEntityType" error={fieldErrors.internalEntityType} />
      </label>
      <label className="text-sm font-medium">{m("攤點通項目")}
      ...
      <label className="text-sm font-medium">{m("外送平台項目 ID")}
      ...
      <label className="text-sm font-medium">{m("外送平台上層 ID（選填）")}
      ...
      <div className="flex flex-wrap items-center gap-3 md:col-span-2">
        <button type="submit" disabled={pending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-teal-700 px-4 font-semibold text-white disabled:opacity-50"><Link2 className="h-4 w-4" />{m("儲存對應")}</button>
