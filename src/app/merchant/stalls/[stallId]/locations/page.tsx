...
export default async function StallLocationsPage({ params }: PageProps) {
  ...
  if (!access.allowed) return <FeatureUpgradeNotice title="出攤地點尚未開放" message={access.message} billingHref={`/merchant/subscription?organizationId=${workspace.id}`} returnHref={`/merchant/stalls/${stallId}`} returnLabel="返回攤位設定" />;
