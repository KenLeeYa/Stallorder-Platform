...
export default async function StallLocationsPage({ params }: PageProps) {
  ...
  if (!access.allowed) return <FeatureUpgradeNotice title={m("出攤地點尚未開放")} message={access.message} billingHref={`/merchant/subscription?organizationId=${workspace.id}`} returnHref={`/merchant/stalls/${stallId}`} returnLabel={m("返回攤位設定")} />;
  const data = await getStallLocationManagerData(workspace.id, stallId);
  return <main className="mx-auto min-h-[calc(100vh-76px)] max-w-4xl px-4 py-7 md:px-8"><ContextualBackButton fallbackHref={`/merchant/stalls/${stallId}`}>{m("返回攤位設定")}</ContextualBackButton><header className="mt-4 border-b border-stone-200 pb-5"><p className="text-sm font-semibold text-teal-800">{workspace.businessName}</p><h1 className="mt-1 flex items-center gap-3 text-3xl font-semibold"><MapPinned className="h-7 w-7 text-teal-700" />{m("常用出攤地點")}</h1><p className="mt-2 text-sm text-stone-600">{stall.name}</p></header><div className="py-7"><StallLocationManager stallId={stallId} initialData={data} /></div></main>;
}
