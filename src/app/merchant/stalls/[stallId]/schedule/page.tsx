...
export default async function StallSchedulePage({ params }: PageProps) {
  ...
  if (!access.allowed) return <FeatureUpgradeNotice title="出攤行程尚未開放" message={access.message} billingHref={`/merchant/subscription?organizationId=${workspace.id}`} returnHref={`/merchant/stalls/${stallId}`} returnLabel="返回攤位設定" />;
  const data = await getStallScheduleManagerData(workspace.id, stallId);
  return <main className="mx-auto min-h-[calc(100vh-76px)] max-w-5xl px-4 py-7 md:px-8"><div className="flex flex-wrap items-center justify-between gap-3"><Link href={`/merchant/stalls/${stallId}`} className="inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-teal-800"><ArrowLeft className="h-4 w-4" />返回攤位設定</Link><Link href={`/s/${stall.slug}/schedule`} target="_blank" className="inline-flex min-h-10 items-center gap-2 rounded-md border border-stone-300 px-3 text-sm font-semibold"><ExternalLink className="h-4 w-4" />公開行程預覽</Link></div><header className="mt-4 border-b border-stone-200 pb-5"><p className="text-sm font-semibold text-teal-800">{workspace.businessName}</p><h1 className="mt-1 flex items-center gap-3 text-3xl font-semibold"><CalendarRange className="h-7 w-7 text-teal-700" />出攤行程與接單時段</h1><p className="mt-2 text-sm text-stone-600">{stall.name}</p></header><div className="py-7"><StallScheduleManager stallId={stallId} initialData={data} /></div></main>;
}
