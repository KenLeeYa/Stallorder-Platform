...
export default async function LineIntegrationPage({ params }: PageProps) {
  ...
  return (
    <main className="mx-auto min-h-[calc(100vh-76px)] max-w-4xl px-4 py-7 md:px-8">
      <Link href={`/merchant/stalls/${stallId}`} className="inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-teal-800"><ArrowLeft className="h-4 w-4" />返回攤位設定</Link>
      <header className="mt-4 border-b border-stone-200 pb-5">
        ...
        <h1 className="mt-1 flex items-center gap-3 text-3xl font-semibold"><MessageCircle className="h-7 w-7 text-emerald-700" />LINE 訂單通知</h1>
