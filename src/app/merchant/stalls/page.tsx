  ...
  return (
    <main className="mx-auto min-h-[calc(100vh-76px)] max-w-5xl px-4 py-7 md:px-8">
      <div className="flex flex-col gap-4 border-b border-stone-200 pb-6 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-sm font-semibold text-teal-800">{workspace.businessName}</p><h1 className="mt-1 text-3xl font-semibold">管理攤位</h1><p className="mt-2 text-sm text-stone-600">停用攤位會保留歷史訂單、商品設定與報表資料。</p></div>
        {canCreate ? <Link href={`/merchant/stalls/new?organizationId=${workspace.id}`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-stone-900 px-4 py-2 text-sm font-semibold text-white"><Plus className="h-4 w-4" />新增攤位</Link> : null}
      </div>
