...
export default async function MerchantDeliveryConnectionPage({ params, searchParams }: PageProps) {
  ...
  const links = [
    { href: "stores", label: "外送門市對應", icon: Store },
    { href: "menu-mapping", label: "商品與註記對應", icon: Link2 },
    { href: "orders", label: "外送訂單", icon: ClipboardList },
    { href: "logs", label: "處理紀錄", icon: ScrollText },
  ];
  return (
    <main className="mx-auto min-h-[calc(100vh-76px)] max-w-4xl px-4 py-7 md:px-8">
      <Link href={`/merchant/integrations/delivery?stallId=${scope.stall.id}`} className="inline-flex min-h-10 items-center gap-2 text-sm font-semibold text-teal-800"><ArrowLeft className="h-4 w-4" />返回外送平台整合</Link>
      <header className="mt-4 border-b border-stone-200 pb-5">
        ...
        <p className="mt-2 text-sm text-stone-600">狀態：{deliveryConnectionStatusLabels[connection.status] ?? connection.status}{connection.externalStoreName ? ` · ${connection.externalStoreName}` : ""}</p>
      </header>
      ...
      <section className="border-t border-stone-200 py-6">
        <h2 className="mb-3 text-lg font-semibold">緊急控制</h2>
