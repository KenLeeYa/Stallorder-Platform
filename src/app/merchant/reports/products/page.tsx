...
export default async function ProductReportPage({ searchParams }: PageProps) {
  ...
  return <main data-testid="report-products" className="mx-auto min-h-[calc(100vh-76px)] max-w-7xl px-4 py-7 md:px-8">
    ...
    <section className="grid gap-8 py-7 lg:grid-cols-2"><div><h2 className="text-xl font-semibold">全組織熱銷商品</h2><div className="mt-3 divide-y divide-stone-100 border-y border-stone-200">{topProducts.map((product, index) => <div key={product.name} className="grid min-w-0 grid-cols-[2rem_minmax(0,1fr)] gap-x-2 gap-y-1 py-3 text-sm sm:grid-cols-[2rem_minmax(0,1fr)_auto] sm:items-center"><span className="row-span-2 text-stone-600 sm:row-span-1">{index + 1}</span><span className="min-w-0 break-words font-medium">{product.name}<span className="ml-2 whitespace-nowrap text-stone-500">{product.quantity} 份</span></span><strong className="col-start-2 break-words tabular-nums sm:col-start-auto">{formatMoney(product.revenue, scope.workspace.defaultCurrency)}</strong></div>)}{topProducts.length === 0 ? <p className="py-8 text-center text-sm text-stone-500">此區間尚無已完成商品銷售。</p> : null}</div></div>
      <div><h2 className="text-xl font-semibold">各攤熱銷商品</h2><div className="mt-3 divide-y divide-stone-200 border-y border-stone-200">{scope.stalls.map((stall) => <div key={stall.id} className="py-4"><h3 className="break-words font-semibold">{stall.name}</h3>{report.products.filter((row) => row.stallId === stall.id).slice(0, 5).map((row) => <div key={row.productName} className="mt-2 grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-2 text-sm"><span className="break-words">{row.productName} · {row.quantity} 份</span><strong className="break-words text-right tabular-nums">{formatMoney(row.revenue, scope.workspace.defaultCurrency)}</strong></div>)}</div>)}</div></div>
    </section>
    <section className="pb-8"><h2 className="text-xl font-semibold">每小時銷售比較</h2>{report.hours.length > 0 ? <ul data-testid="product-hourly-dashboard" className="mt-3 grid grid-cols-2 overflow-hidden rounded-lg border border-stone-200 bg-white min-[360px]:grid-cols-3 sm:grid-cols-4 lg:grid-cols-6">{report.hours.map((row) => <li key={`${row.stallId}-${row.hour}`} className="min-w-0 border-b border-r border-stone-200 bg-white p-3 text-sm"><p className="font-semibold tabular-nums text-teal-800">{String(row.hour).padStart(2, "0")}:00</p><p className="mt-1 break-words text-xs text-stone-600">{row.stallName}</p><p className="mt-2 tabular-nums text-stone-500">{row.orderCount} 筆</p><strong className="mt-1 block break-words tabular-nums">{formatMoney(row.sales, scope.workspace.defaultCurrency)}</strong></li>)}</ul> : <p className="mt-3 border-y border-stone-200 py-8 text-center text-sm text-stone-500">此區間尚無時段資料。</p>}</section>
  </main>;
}
