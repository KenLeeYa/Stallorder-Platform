...
const collapsibleClass = "group border-y border-stone-200";
const summaryClass = "flex cursor-pointer list-none items-center justify-between gap-3 py-4 [&::-webkit-details-marker]:hidden";
const chevronClass = "h-5 w-5 shrink-0 text-stone-500 transition-transform group-open:rotate-180";
...
export default async function ProductReportPage({ searchParams }: PageProps) {
  ...
  const organizationGroups = new Map<string, { categoryName: string; groupName: string; quantity: number; revenue: number }>();
  for (const row of report.groups) {
    const key = `${row.categoryName}\u0000${row.groupName}`;
    const current = organizationGroups.get(key) ?? { categoryName: row.categoryName, groupName: row.groupName, quantity: 0, revenue: 0 };
    organizationGroups.set(key, { ...current, quantity: current.quantity + row.quantity, revenue: current.revenue + row.revenue });
  }
  const topGroups = [...organizationGroups.values()].sort((a, b) => b.revenue - a.revenue);
  return <main data-testid="report-products" className="mx-auto min-h-[calc(100vh-76px)] max-w-7xl px-4 py-7 md:px-8">
    ...
    <ReportNavigation organizationId={scope.workspace.id} active="products" /><ReportFilters organizationId={scope.workspace.id} stalls={scope.availableStalls} selectedStallIds={scope.stalls.map((stall) => stall.id)} dateFrom={scope.dateFrom} dateTo={scope.dateTo} multiStallMode={scope.workspace.operatingMode === "MULTI_STALL"} />
    <section className="pt-7"><details open data-testid="product-group-sales-section" className={collapsibleClass}><summary className={summaryClass}><h2 className="text-xl font-semibold">{t("reports.products.groups")}</h2><span aria-hidden="true" className={chevronClass}>⌄</span></summary><div className="divide-y divide-stone-100 border-t border-stone-200">{topGroups.map((group, index) => <div key={`${group.categoryName}-${group.groupName}`} className="grid min-w-0 grid-cols-[2rem_minmax(0,1fr)] gap-x-2 gap-y-1 py-3 text-sm sm:grid-cols-[2rem_minmax(0,1fr)_auto] sm:items-center"><span className="row-span-2 text-stone-600 sm:row-span-1">{formatAppNumber(locale, index + 1)}</span><span className="min-w-0 break-words font-medium"><span className="text-stone-500">{group.categoryName}／</span>{group.groupName}<span className="ml-2 whitespace-nowrap text-stone-500">{t("reports.count.items", { count: formatAppNumber(locale, group.quantity) })}</span></span><strong className="col-start-2 tabular-nums sm:col-start-auto">{formatAppCurrency(locale, group.revenue, scope.workspace.defaultCurrency, { maximumFractionDigits: 0 })}</strong></div>)}{topGroups.length === 0 ? <p className="py-8 text-center text-sm text-stone-500">{t("reports.products.none")}</p> : null}</div></details></section>
    <section className="grid gap-8 py-7 lg:grid-cols-2"><details open data-testid="organization-top-products-section" className={collapsibleClass}><summary className={summaryClass}><h2 className="text-xl font-semibold">{t("reports.products.organizationTop")}</h2><span aria-hidden="true" className={chevronClass}>⌄</span></summary><div className="divide-y divide-stone-100 border-t border-stone-200">{topProducts.map((product, index) => <div key={product.name} className="grid min-w-0 grid-cols-[2rem_minmax(0,1fr)] gap-x-2 gap-y-1 py-3 text-sm sm:grid-cols-[2rem_minmax(0,1fr)_auto] sm:items-center"><span className="row-span-2 text-stone-600 sm:row-span-1">{formatAppNumber(locale, index + 1)}</span><span className="min-w-0 break-words font-medium">{product.name}<span className="ml-2 whitespace-nowrap text-stone-500">{t("reports.count.items", { count: formatAppNumber(locale, product.quantity) })}</span></span><strong className="col-start-2 break-words tabular-nums sm:col-start-auto">{formatAppCurrency(locale, product.revenue, scope.workspace.defaultCurrency, { maximumFractionDigits: 0 })}</strong></div>)}{topProducts.length === 0 ? <p className="py-8 text-center text-sm text-stone-500">{t("reports.products.none")}</p> : null}</div></details>
      <details open data-testid="stall-top-products-section" className={collapsibleClass}><summary className={summaryClass}><h2 className="text-xl font-semibold">{t("reports.products.stallTop")}</h2><span aria-hidden="true" className={chevronClass}>⌄</span></summary><div className="divide-y divide-stone-200 border-t border-stone-200">{scope.stalls.map((stall) => <div key={stall.id} className="py-4"><h3 className="break-words font-semibold">{stall.name}</h3>{report.products.filter((row) => row.stallId === stall.id).slice(0, 5).map((row) => <div key={row.productName} className="mt-2 grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-2 text-sm"><span className="break-words">{row.productName} · {t("reports.count.items", { count: formatAppNumber(locale, row.quantity) })}</span><strong className="break-words text-right tabular-nums">{formatAppCurrency(locale, row.revenue, scope.workspace.defaultCurrency, { maximumFractionDigits: 0 })}</strong></div>)}</div>)}</div></details>
    </section>
