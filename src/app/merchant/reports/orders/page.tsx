...
import { getOrderHistoryReport } from "@/lib/report-data";
...
export default async function OrderHistoryReportPage({ searchParams }: PageProps) {
  ...
  const orders = await getOrderHistoryReport(
    scope.workspace.id,
  ...
  return <main data-testid="report-orders" className="mx-auto min-h-[calc(100vh-76px)] max-w-7xl px-4 py-7 md:px-8">
    ...
    <section className="py-7">
      <p className="mb-3 text-sm font-semibold text-stone-700">{t("reports.count.orders", { count: formatAppNumber(locale, orders.length) })}</p>
      ...
      <p className="mt-3 text-xs text-stone-500">{t("reports.orders.limit")}</p>
    </section>
  </main>;
}
