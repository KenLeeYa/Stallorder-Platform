...
export default async function MarketEventsPage({ searchParams }: PageProps) {
  ...
  if (!access.allowed) return <FeatureUpgradeNotice title="市集活動尚未開放" message={access.message} billingHref={`/merchant/subscription?organizationId=${workspace.id}`} />;
  let data;
  try {
