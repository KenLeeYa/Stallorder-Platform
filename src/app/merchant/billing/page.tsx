...
export default async function MerchantBillingPage({ searchParams }: PageProps) {
  ...
  return (
    <main className="mx-auto min-h-[calc(100vh-76px)] max-w-6xl px-4 py-7 md:px-8">
      <BillingPageHeader organizationName={workspace.businessName} organizationId={workspace.id} active="billing" title="訂閱與帳務" description="檢視目前方案、付款狀態、待辦申請與帳務通知。" />
      ...
      <section className="grid border-b border-stone-200 sm:grid-cols-2 lg:grid-cols-4">
        <Metric label="目前方案" value={`${subscription.planVersion.displayName} v${subscription.planVersion.version}`} />
        <Metric label="訂閱狀態" value={subscriptionStatusLabels[subscription.status] ?? subscription.status} />
        <Metric label="本期訂單" value={`${usage.billableOrders} / ${orderLimit ?? "依合約"}`} />
        <Metric label="有效攤位" value={`${usage.activeStalls} / ${subscription.planVersion.maxStalls ?? "依合約"}`} />
        <Metric label="計費期間" value={`${dateOnly(subscription.billingPeriodStart)} 至 ${dateOnly(subscription.billingPeriodEnd)}`} />
        <Metric label="試用到期" value={subscription.trialEndsAt ? dateOnly(subscription.trialEndsAt) : "不適用"} />
        <Metric label="下次到期日" value={subscription.paymentDueAt ? dateOnly(subscription.paymentDueAt) : "待平台設定"} />
        <Metric label="付款週期" value={subscription.billingInterval === "ANNUAL" ? "年繳" : subscription.billingInterval === "MONTHLY" ? "月繳" : "試用"} />
      </section>

      {warnings.length > 0 ? <section className="border-b border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-950"><h2 className="font-semibold">用量提醒</h2><p className="mt-1">目前已使用 {warnings.at(-1)?.percentage}% 訂單額度；付費方案預設採軟性上限，不會在營業中突然停止接單。</p></section> : null}
      ...
      <section className="border-t border-stone-200 py-6"><h2 className="text-xl font-semibold">可用訂單包</h2><div className="mt-3 divide-y divide-stone-200 border-y border-stone-200">{data.orderPackages.map((item) => <div key={item.id} className="flex flex-wrap justify-between gap-3 py-3 text-sm"><span><strong>{item.displayName}</strong><span className="ml-2 text-stone-500">需平台人工指派</span></span><strong>{formatMoney(item.unitPrice, item.currency)}</strong></div>)}</div></section>

      <section className="border-t border-stone-200 py-6"><h2 className="text-xl font-semibold">帳務通知</h2><div className="mt-3 divide-y divide-stone-200 border-y border-stone-200">{notifications.slice(0, 8).map((notification) => <article key={notification.id} className="py-3"><div className="flex justify-between gap-3"><strong className="text-sm">{notification.title}</strong><time className="text-xs text-stone-500">{notification.createdAt.toISOString().slice(0, 10)}</time></div><p className="mt-1 text-sm text-stone-600">{notification.message}</p></article>)}{notifications.length === 0 ? <p className="py-6 text-sm text-stone-500">目前沒有帳務通知。</p> : null}</div></section>
