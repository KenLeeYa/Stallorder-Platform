...
export default async function AdminInvoiceDetailPage({ params }: PageProps) {
  ...
  return (
    <main className="mx-auto min-h-[calc(100vh-76px)] max-w-5xl px-4 py-7 md:px-8">
      <Link href="/admin/invoices" className="text-sm font-semibold text-teal-800">{m("Back to invoices")}</Link>
      ...
      <section className="py-6"><h2 className="text-xl font-semibold">{m("Invoice details")}</h2><div className="mt-3 overflow-x-auto border-y border-stone-200"><table className="w-full min-w-[640px] text-left text-sm"><thead className="bg-stone-50"><tr><th className="px-3 py-3">{m("Item")}</th><th className="px-3 py-3">{m("Type")}</th><th className="px-3 py-3 text-right">{m("Quantity")}</th><th className="px-3 py-3 text-right">{m("Unit price")}</th><th className="px-3 py-3 text-right">{m("Subtotal")}</th></tr></thead><tbody className="divide-y divide-stone-200">{invoice.lineItems.map((line) => <tr key={line.id}><td className="px-3 py-4 font-medium">{line.description}</td><td className="px-3 py-4 text-stone-600">{getAdminCodeLabel(locale, line.itemType)}</td><td className="px-3 py-4 text-right">{formatAppNumber(locale, line.quantity)}</td><td className="px-3 py-4 text-right">{formatAppCurrency(locale, line.unitPrice, invoice.currency)}</td><td className="px-3 py-4 text-right">{formatAppCurrency(locale, line.subtotal, invoice.currency)}</td></tr>)}</tbody></table></div><dl className="ml-auto mt-4 max-w-sm space-y-2 text-sm"><Amount locale={locale} label={m("Subtotal")} value={invoice.subtotal} currency={invoice.currency} /><Amount locale={locale} label={m("Credits / discounts")} value={invoice.discountAmount} currency={invoice.currency} /><Amount locale={locale} label={m("Total")} value={invoice.totalAmount} currency={invoice.currency} strong /><Amount locale={locale} label={m("Paid")} value={invoice.amountPaid} currency={invoice.currency} /><Amount locale={locale} label={m("Due")} value={invoice.amountDue} currency={invoice.currency} strong /></dl></section>
...
function Amount({ locale, label, value, currency, strong = false }: { locale: AppLocale; label: string; value: number; currency: string; strong?: boolean }) {
  return <div className="flex justify-between gap-3"><dt className={strong ? "font-semibold" : "text-stone-600"}>{label}</dt><dd className={strong ? "font-semibold" : ""}>{formatAppCurrency(locale, value, currency)}</dd></div>;
}
