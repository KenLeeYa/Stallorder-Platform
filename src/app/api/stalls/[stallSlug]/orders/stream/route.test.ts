...
describe("staff order SSE admission", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 429 without polling when the stream lease is full", async () => {
    ...
    const response = await route.GET(new Request("https://example.test/api/stalls/demo/orders/stream"), {
      params: Promise.resolve({ stallSlug: "demo" }),
    });
    expect(response.status).toBe(429);
    expect(mocks.acquireLease).toHaveBeenCalledWith({
      profileId: "profile-1", stallId: "stall-1", streamKind: "orders",
    });
