...
async function getLatestKitchenEventId(stallId: string) {
...
export async function GET(request: Request, context: RouteContext) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      const poll = async () => {
        try {
          const currentEventId = await getLatestKitchenEventId(authorization.stall.id);
          ...
          logEvent("error", "KDS_STREAM_POLL_FAILED", {
