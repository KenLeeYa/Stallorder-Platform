...
import { additionalStallRequestSchema } from "@/server/billing/billing-validation";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsed = additionalStallRequestSchema.safeParse(body.data);
  if (!parsed.success) {
    return NextResponse.json({ error: parsed.error.issues[0]?.message ?? "額外攤位申請資料不正確。" }, { status: 400, headers: { "x-request-id": authorization.requestId } });
  }
  try {
    const changeRequest = await billingWorkflowService.requestAdditionalStalls(
      organizationId,
      parsed.data,
