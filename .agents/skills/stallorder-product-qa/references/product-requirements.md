...
- `QR-023` Checkout add-on recommendation is a per-stall module that defaults off. Each stall product has a separate recommendation switch in stall-product settings and defaults unselected. The centered recommendation dialog appears once before checkout only when the module is enabled and at least one configured product is published, enabled, not sold out, and not already in the cart. Skip continues checkout; add uses the normal customization flow; server-side catalog and price validation remains authoritative. A stall may configure at most six recommendation products.
...
- `CAT-004` Shared notes are first-class reusable records. Note groups select multiple existing shared notes through a centered, scrollable modal with search, multi-select, visible confirm/cancel, deduplication, and responsive layout.
- `CAT-005` Notes, note groups, categories, groups, products, and relevant catalog groups support deterministic ordering. The note-group collapse control appears only on the note-group tab and aligns on the same row.
...
- `ADM-005` Local development login exposes four explicit role buttons for fast testing only. Production builds MUST not render, accept, or route through local test-login credentials or origin bypasses.
- `ADM-006` Account login origin/CSRF validation accepts configured localhost and same-LAN local test origins only in local development and remains fail-closed elsewhere.
