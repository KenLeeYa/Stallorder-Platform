...
- `QR-016` Public takeout and delivery require customer name and reachable phone before submission. Validation is server-enforced and identifies the missing field in the active locale. Returning to amend an existing order restores the original customer name and phone, and a successful amendment preserves that contact on the same order.
...
- `QR-020` Before merchant confirmation, a customer may cancel or modify the submitted order. After confirmation but before production policy locks it, changes use an amendment flow on the same order, recompute totals server-side, and notify Staff again. The edit draft is scoped to the tracked order and returns to its original QR/takeout/delivery Menu rather than opening a new service-mode order. Once locked, the UI explains why a new order is required.
...
- `MER-014` On desktop, QR management displays QR codes at a bounded, scannable size rather than scaling to fill the viewport. At tablet width the management content uses the available page width in two columns: QR/print controls on the left and the unified public link/actions on the right; phone remains a readable single column.
