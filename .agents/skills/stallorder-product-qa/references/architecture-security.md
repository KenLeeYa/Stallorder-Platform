...
- Create the Plan-bound Cloudflare CNAME as DNS-only (`proxied=false`) and read back its exact record ID, name, type, and normalized content. Do not create the record as proxied or replace it with a second record while Vercel validates the direct target.
- Wait until Vercel reports the custom domain with `misconfigured=false`; only then PATCH that same Cloudflare record ID to `proxied=true` and read back the unchanged identity plus `proxied=true` before testing the protected hostname.
...
- If a protected DR Apply fails after remote mutations begin, roll back and read back every resource by its exact recorded identity. The tracked CNAME may legitimately be in either the DNS-only or proxied phase, but rollback accepts either only when record ID, name, type, and normalized content all match. A generic cleanup result is not proof that Vercel, domain/DNS, Cloudflare Access, QA-token, or prior-binding state was restored or removed as intended.
...
- Public-order token derivation, abuse hashing, and Turnstile verification use verified originals through masked release automation. The application and its Edge functions receive the same values. Primary/DR database and provider credentials remain isolated; the three public-order runtime secrets are deliberately synchronized under the current protected DR Plan to preserve replicated customer-order continuity. DR readiness requires target digest readback and reviewed Edge deployment, not only database catch-up. No secret value or masking directive enters logs, plans, artifacts, or client bundles.
- Lost public-order originals may be recovered only through the reviewed `production-public-order-secret-recovery` operation: current owner/Plan/commit/tree/project/digest/recipient binding, JWT plus service-role and random nonce, five-minute maximum lifetime, AES-GCM envelope wrapped to a local-only RSA private key, and exact temporary-function deletion readback before artifact publication. The service-role authorization fingerprint must come from the trusted management API key used for that invocation, not a potentially different built-in runtime key. Plans contain fingerprints only; artifacts contain authenticated ciphertext only. Plaintext originals never enter logs, plans, artifacts, or client bundles. Recovery does not rotate keys or alter customer orders.
