...
| 滿版登入場景 | 商家與員工的真實登入控制項位於圖片平板顯示區；小螢幕、多登入方式與文字放大時可在顯示區捲動。 | 將控制項烘焙成圖片、裁切到無法操作，或為版面改動放寬登入 policy／next／session 檢查。 |
...
| 註記管理 | 手機主頁保留「單一註記」「註記群組」兩個大入口；平板／電腦直接顯示左群組、右選項，入口改為精簡按鈕。搜尋與新增跟隨目前看板，編輯開啟置中視窗。 | 保留重複頂部工具列／頁籤，或把單一註記攤成長清單。 |
...
| DR Vercel domain | 先以 DNS-only 建立 Plan 綁定的 CNAME 並核對精確身分；等 Vercel `misconfigured=false` 後，才把同一 record ID PATCH 為 proxied 並再核對身分與狀態。Rollback 可接受該筆紀錄暫時為 DNS-only 或 proxied，但 ID／name／type／content 必須全部吻合。 | 一開始就開 proxy、另建或改用第二筆紀錄、只靠名稱找紀錄，或 rollback 因過渡中的 proxy 狀態而拒絕合法的精確紀錄。 |
...
顧客／店員時間協調的 QA 需包含真實閒置、多分頁、慢回應、斷線、過期、回應遺失及同時取消。短時間的一次接受／拒絕不能證明輪詢配額與版本競爭正確。執行 `QA-QR-15`、`QA-QR-16`，參考 `docs/CUSTOMER_ORDER_FUNCTIONAL_QA_20260907.md`。背景同步保留行內狀態；成功回覆與提示綁定版本；提交時的 server 狀態判斷優先於畫面按鈕。
...
- 手機單一註記與註記群組沿用「大入口 → 搜尋／分層清單 → 置中操作」；平板／電腦必須直接顯示左群組／右選項，不能只修改放大視窗；320／390／768／1440 都要能看見標題、搜尋、關閉與操作，且整頁不得水平溢位。
- 單一註記子視窗不能關閉父層清單或清空搜尋；父清單維持原捲動位置，子視窗獨占 Escape／Tab，回饋關閉後回到清單。返回驗證必須直接斷言清單仍存在，不能由測試 helper 重開入口後才檢查。
...
- QR 管理在 768px 使用滿寬雙欄（左 QR／印刷、右公開連結／動作），手機維持單欄；A4／A5／A6 按鈕的紙張尺寸與「印刷版」固定為兩行，列印頁不得包含商家後台殼層，列印／存成 PDF 必須實際觸發瀏覽器列印流程。
...
- Custom domain 的 CNAME 先以 `proxied=false` 建立，並核對 record ID、name、type 與正規化 content；Vercel 回報 `misconfigured=false` 後，只 PATCH 同一 record ID 為 `proxied=true`，再核對相同身分與 proxy 狀態。
- Apply 在兩階段之間失敗時，rollback 對該精確 CNAME 可接受 `proxied=false` 或 `proxied=true`，但不能只靠名稱匹配，也不能接受 ID／type／content 不符；復原或刪除後仍需獨立 read-back 最終狀態。
...
- 「全功能啟用」仍須保留資料庫對未實作功能的硬鎖；本機支付、寄送與外部供應商僅使用模擬能力。不得因為要測試而解除正式環境安全界線。
- 抽獎主開關關閉後，既存活動不能繼續擋住送單；內用桌位的抽獎入口、底層 draw RPC、送單交易與贈品折扣快照必須一致。
- 金流操作失敗必須顯示可關閉的錯誤視窗並解除 pending。回應遺失的重試沿用同一付款操作識別，不能新增第二筆交易。
- 工作階段驗證錯誤與一般系統降級遮罩不能重疊阻擋關閉或重試；明確錯誤優先，持續狀態與重新檢查保留在頁面。
- Node 與 Edge 的公開訂單時間欄位皆須含明確時區；不含 offset 的資料庫 UTC 時間不可直接交給瀏覽器當成本地時間。
- 編譯模式的本機 E2E 必須在 build 階段帶入公開測試 Turnstile site key；僅在 next start 設定不能更新已編譯的瀏覽器常數。手動 QA 最後還原 development 啟動器與四角色快速登入。
