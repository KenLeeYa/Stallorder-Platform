...
| `QA-ORD-10` | `STAFF-019`,`MER-013` | At tablet/desktop widths, queue, selected item detail, and action rail appear left-to-right; all three panes independently scroll, the item pane contains item/price/fulfillment controls without repeating the selected order identity or fulfillment summary, and the page has no horizontal overflow. Checkout, print, edit, status, and cancellation remain reachable in the right rail under their existing permission/state gates. Phone keeps a single-column order-first flow with reachable detail and centered action dialogs. |
