...
| `QA-QR-08` | `QR-019`,`QR-020` | Return-to-Menu revalidates availability/version; the edit draft remains scoped to the tracked order and original service-mode route; pre-confirm cancel and pre-lock amendment update that same order, while locked orders remain unchanged and Staff is re-notified. Simulate an application/canonical device-hash mismatch and verify recovery succeeds only after exact canonical device/origin/IP/protocol/operation validation; wrong device or unavailable validator fails closed. |
