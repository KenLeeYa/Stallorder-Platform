...
6. Create the Plan-bound Cloudflare CNAME as DNS-only (`proxied=false`) with a 60-second TTL and read back the same record ID, name, type, normalized content and TTL before asking Vercel to validate the custom domain. Plan schema 4 binds `dnsOnlyTtlSeconds=60`; reject missing or different values. DNS-only Auto (300 seconds) can outlast the proxy handoff wait. The final proxied record uses Cloudflare Auto TTL.
...
8. Run any authenticated generated-deployment probe through `vercel curl` with the workflow-scoped `VERCEL_TOKEN`; never pass an explicit `--token`, which Vercel CLI 56.3.1 forwards to the underlying curl process.
   ...
   Close direct-origin connections before enabling the DNS proxy. Custom-domain readiness must confirm Cloudflare ingress as well as anonymous denial before sending QA credentials; an origin 403 is still pending. Probe with fresh connections under the existing bounded wait, reject non-JSON success, and retain only sanitized transport evidence. Do not relax JWT verification or extend timeouts to mask a failed handoff.
