...
- **Sold-out catalog presentation**: the latest instruction supersedes the earlier hide/filter behavior. Public Menu, QR, takeout, and delivery keep published/enabled sold-out items visible with a grayscale image and localized sold-out label, block customer ordering, and preserve the product for authorized Staff onsite ordering.
- **Checkout add-on recommendation**: both the per-stall module and each stall-product recommendation selection default off. Product selection belongs in stall-product settings, while the module page only enables the flow; only eligible products not already in the cart appear once before checkout.
