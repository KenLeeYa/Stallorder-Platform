...
create unique index print_jobs_amendment_destination_unique on public.print_jobs(
  order_id, amendment_id, coalesce(print_rule_id::text, 'default'), coalesce(printer_id::text, 'default')
) where amendment_id is not null and reprint_of_id is null;
comment on column public.print_jobs.amendment_id is 'Order event identifying an immutable amendment; original print success never satisfies an unprinted amendment.';
...
begin
  ...
  on conflict (order_id, print_rule_id)
    where print_rule_id is not null
      and (reprint_of_id is null or is_routing_copy) and amendment_id is null
    do nothing;
