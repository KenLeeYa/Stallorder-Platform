...
export function statusForCode(code: string) {
  ...
  if (["QR_REVOKED", "QR_PAUSED", "QR_EXPIRED", "QR_NOT_ACTIVE", "TABLE_UNAVAILABLE", "QR_SESSION_MISMATCH", "STALL_CLOSED", "STALL_SPECIAL_CLOSURE", "ORDERING_PAUSED", "STALL_SOLD_OUT", "TENANT_INACTIVE", "SESSION_EXPIRED", "SESSION_REPLAYED", "SESSION_DEVICE_MISMATCH", "SESSION_TOKEN_COLLISION", "DELIVERY_UNAVAILABLE", "ORDER_MODE_CONFLICT", "CAPACITY_PAUSED", "PRODUCT_CAPACITY_EXCEEDED", "LOCATION_UNAVAILABLE", "EVENT_NOT_ACTIVE", "EVENT_EXPIRED", "SCHEDULE_NOT_ACTIVE", "SCHEDULE_CLOSED", "SCHEDULE_CONTEXT_MISMATCH", "PREORDER_DISABLED", "PREORDER_TIME_INVALID", "PREORDER_TIME_UNAVAILABLE", "PREORDER_CONTEXT_UNAVAILABLE", "FULFILLMENT_TIME_UNAVAILABLE", "FULFILLMENT_TIME_PROPOSAL_STALE", "FULFILLMENT_TIME_PROPOSAL_EXPIRED", "LOTTERY_UNAVAILABLE", "LOTTERY_NOT_ELIGIBLE", "LOTTERY_DRAW_INVALID", "LOTTERY_DRAW_EXPIRED", "LOTTERY_ALREADY_REDEEMED", "IDEMPOTENCY_CONFLICT", "ORDER_ALREADY_CONFIRMED", "ORDER_ALREADY_STARTED", "PRINT_ALREADY_STARTED", "PAYMENT_ALREADY_RECORDED", "DISCOUNT_ALREADY_APPLIED"].includes(code)) return 409;
